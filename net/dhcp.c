...
static int dhcpstat_proc(void *arg, int len,struct proc_entry *pf)
{
  ...
  while (state)
  {
    time_t lease_age = get_unixtime(NULL) - state->bind_time;

	if (state->offered_dns1_addr.addr)
	{
    pprintf(pf, "nameserver %s\n", inetntoa(state->offered_dns1_addr.addr));
	}
else{
   pprintf(pf, "nameserver %s\n", inetntoa(state->server_ip_addr.addr));
	}

 	if (state->offered_dns2_addr.addr)
   pprintf(pf, "nameserver %s\n", inetntoa(state->offered_dns2_addr.addr));
	else{
   pprintf(pf, "nameserver %s\n", inetntoa(state->offered_gw_addr.addr));
	}
 ...
 //   pprintf(pf, "domain %s\n", state->offered_domain_name);
    ...
    pprintf(pf, "#  DHCP Server ............ : %s\n", inetntoa(state->server_ip_addr.addr));
    pprintf(pf, "#  IP Adress .............. : %s\n", inetntoa(state->offered_ip_addr.addr));
    pprintf(pf, "#  Subnet Mask ............ : %s\n", inetntoa(state->offered_sn_mask.addr));
    pprintf(pf, "#  Gateway Adress ......... : %s\n", inetntoa(state->offered_gw_addr.addr));
    pprintf(pf, "#  Broadcast Address ...... : %s\n", inetntoa(state->offered_bc_addr.addr));
    pprintf(pf, "#  Primary DNS Server ..... : %s\n", inetntoa(state->offered_dns1_addr.addr));
    pprintf(pf, "#  Secondary DNS Server ... : %s\n", inetntoa(state->offered_dns2_addr.addr));
    pprintf(pf, "#  Primary NTP Server ..... : %s\n", inetntoa(state->offered_ntpserv1_addr.addr));
    pprintf(pf, "#  Secondary NTP Server ... : %s\n", inetntoa(state->offered_ntpserv2_addr.addr));
...
{
  int msecs = 10 * 1000;  
  //mod_timer(&state->request_timeout_timer,  msecs / MSECS_PER_TICK);
  sys_timeout(msecs, dhcp_timeout, state);
...
static void dhcp_check(struct dhcp_state *state)
{
  ...
  if (p != NULL)
  {
    ...
    result = dev_transmit((dev_prvi_t*) state->netif->state, p);
    pbuf_free(p);
    //return result;
  }
  state->tries++;
...
static void dhcp_timeout(void *arg)
{
  ...
  dhcp_timeout_handler(state);

  //queue_task( &state->request_timeout_task, (taskproc_t) dhcp_timeout_handler, state);
}
...
static void dhcp_t1_timeout(struct dhcp_state *state)
{
  if (state->state == DHCP_REQUESTING || state->state == DHCP_BOUND || state->state == DHCP_RENEWING)
  {
	...
	dhcp_renew(state);
    //queue_task( &state->t1_timeout_task, (taskproc_t) dhcp_renew, state);
  }
}
...
static void dhcp_t2_timeout(struct dhcp_state *state)
{
  if (state->state == DHCP_REQUESTING || state->state == DHCP_BOUND || state->state == DHCP_RENEWING)
  {
    ...
    //queue_task(&state->t2_timeout_task, (taskproc_t) dhcp_rebind, state);
	dhcp_rebind(state);
  }
}
...
static void dhcp_handle_ack(struct dhcp_state *state)
{
  ...
  state->bind_time = get_unixtime(NULL);
...
// dhcp_init
//
static struct proc_entry e_dhcpstat_proc = {
	name: "resolv.conf",
	write_func: dhcpstat_proc,
	read_func: NULL,
};
...
void dhcp_init()
{
  register_proc_entry(&e_dhcpstat_proc);
}
...
struct dhcp_state *dhcp_start(struct netif *netif)
{
  ...
  state = kmalloc(sizeof(struct dhcp_state),0);
  ...
  state->netif = netif;
  state->next = NULL;
  state->binding_complete=create_semaphore("dhcp_client",0,0);
  if (state->binding_complete<0)
  {
	  kprintf("binding_complete error\n");
	  return NULL;
  }
...
static err_t dhcp_discover(struct dhcp_state *state)
{
  ...
  if (result == 0)
  {
    ...
    if (udp_send(state->pcb, state->p_out, NULL, 0, state->netif) >= 0)
		state->p_out = NULL;
    udp_bind(state->pcb, IP_ADDR_ANY, DHCP_CLIENT_PORT);
    udp_connect(state->pcb, IP_ADDR_ANY, DHCP_SERVER_PORT);
...
static void dhcp_bind(struct dhcp_state *state)
{
  ...
  {
    //mod_timer(&state->t1_timeout_timer,  state->offered_t1_renew * TICKS_PER_SEC);
	sys_timeout(state->offered_t1_renew*1000, dhcp_t1_timeout, state);
  }

  if (state->offered_t2_rebind != 0xFFFFFFFF)
  {
	  sys_timeout(state->offered_t2_rebind*1000, dhcp_t2_timeout, state);
    //mod_timer(&state->t2_timeout_timer,  state->offered_t2_rebind * TICKS_PER_SEC);
  }
  ...
  unlock_semaphore(state->binding_complete);
...
void dhcp_stop(struct netif *netif)
{
  ...
  if (state)
  {
    ...
    //remove_timer(&state->request_timeout_timer);
	sys_untimeout(dhcp_timeout, state);
	sys_untimeout(dhcp_t1_timeout, state);
	sys_untimeout(dhcp_t2_timeout, state);

    //remove_timer(&state->t1_timeout_timer);
    //remove_timer(&state->t2_timeout_timer);
...
static err_t dhcp_unfold_reply(struct dhcp_state *state)
{
  ...
  if (state->p->tot_len > sizeof(struct dhcp_msg))
  {
    ...
    state->options_in = kmalloc(state->options_in_len,0);
    if (!state->options_in) return ENOMEM;
  }

  state->msg_in = kmalloc(sizeof(struct dhcp_msg),0);
  if (!state->msg_in)
  {
...
static err_t dhcp_create_request(struct dhcp_state *state)
{
  ...
  for (i = 0; i < ETHER_ADDR_LEN; i++)
	  state->msg_out->chaddr[i] = state->netif->hwaddr.addr[i];
