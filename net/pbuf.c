...
void pbuf_init()
{
  ...
  pbuf_pool = (struct pbuf *) 
	  kmalloc(PBUF_POOL_SIZE * (PBUF_POOL_BUFSIZE + sizeof(struct pbuf)),0);
...
struct pbuf *pbuf_alloc(int layer, int size, int flag)
{
  ...
  switch (flag) 
  {
    ...
    case PBUF_RW:
      ...
      p = (struct pbuf *) kmalloc(sizeof(struct pbuf) + size + offset,0);
    ...
    case PBUF_RO:
      ...
      p = (struct pbuf *) kmalloc(sizeof(struct pbuf),0);
      if (p == NULL) return NULL;
