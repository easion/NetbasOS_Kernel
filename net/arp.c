...
static int arp_proc(void *arg, int len,struct proc_entry *pf)
{
  int i;
  char buf[32];
  ...
  for (i = 0; i < ARP_TABLE_SIZE; ++i)
  {
    ...
    {
      pprintf(pf, "%d: %s %s\n", i,ether2str(&arp_table[i],buf), inetntoa(arp_table[i].ipaddr.addr));
    }
  }
...
static void arp_tmr(void *arg)
{
  ...
  sys_timeout(ARP_TIMER_INTERVAL, arp_tmr, NULL);
}

static struct proc_entry e_arp_proc = {
	name: "arp",
	write_func: arp_proc,
	read_func: NULL,
};
...
void arp_init()
{
  ...
  sys_timeout(ARP_TIMER_INTERVAL, arp_tmr, NULL);
  register_proc_entry(&e_arp_proc);
}
...
static void add_arp_entry(struct ip_addr *ipaddr, struct eth_addr *ethaddr)
{
  ...
  for (i = 0; i < ARP_XMIT_QUEUE_SIZE; i++)
  {
	...
	ethhdr->src.addr[i] = entry->netif->hwaddr.addr[i];
      ...
      err = dev_transmit((dev_prvi_t*) entry->netif->state, p);
      if (err < 0)
      {
