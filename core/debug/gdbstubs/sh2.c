...
/* $Id: sh2.c,v 1.2 2002/04/15 19:37:39 bgat Exp $ */
...
int gdb_poke_register_file (int id, long val)
	...
	switch( id )
		...
		case VBR: (long)register_file.vbr = val; break;
...
int gdb_trapa34 (int syscall, int arg1, int arg2, int arg3)
	...
	switch (syscall)
		{
		case 4:
		  gdb_console_output(arg3, (char*)arg2);
		  break;
		}
	return 0;
	}
...
/* OBSOLETE and TODO-type stuff */

/*
  Uses the UBC to generate an exception
  after we execute the next instruction.

  This code is hopelessly broken, because according to how
  I read the hardware manual section 7.5, the UBC won't generate
  exceptions on instructions that are branch targets.  This makes
  it a pretty useless peripheral, as far as I can tell.

  21sep00:  I have an idea that maybe you could run the UBC in
  fetch/data access mode, and somehow detect when it has hung up
  due to interrupt (i.e. we're re-executing the same address over
  and over again).  But I don't have time to look into this right now.
*/
void gdb_step ( long addr )
	{
	...
	/* Use the UBC to break on instruction fetch (before execution).
     ...
     others), or if the target instruction is in a delay slot.
  ...
  */
  ...
  /* determine where the target instruction will send us to */
	...
	/* set the breakpoint address there */
	*SH704x_UBC_UBARH = (short)( addr >> 16 );
	*SH704x_UBC_UBARL = (short)( addr );
	*SH704x_UBC_UBAMRH = 0;
	*SH704x_UBC_UBAMRL = 0;

  /* break on instruction fetches, see section 7.4.1
     of the User's Manual (2nd ed) for more on this */
#if 1
	*SH704x_UBC_UBBR = 0x54;
#else
	*SH704x_UBC_UBBR = 0x74;
#endif
...
__asm__("   .global _gdb_ubc_isr                ");
__asm__("   _gdb_ubc_isr:                       ");

__asm__("   /* put r0, r1 on the stack */       ");
__asm__("   mov.l r0, @-r15                     ");
__asm__("   mov   #0xf0, r0                     ");
__asm__("   ldc   r0, sr                        ");
__asm__("   mov.l r1, @-r15                     ");

__asm__("   /* put SIGTRAP on stack */          ");
__asm__("   mov #5, r0                          ");
__asm__("   mov.l r0, @-r15                     ");

__asm__("   /* zero UBBR to turn off UBC */     ");
__asm__("   mov.l ubc_ubbr, r0                  ");
__asm__("   mov #0, r1                          ");
__asm__("   mov.w r1, @r0                       ");

__asm__("   /* save registers,                  ");
__asm__("      call gdb_handle_exception */     ");
__asm__("   bra save_registers_handle_exception ");
__asm__("   nop                                 ");

__asm__("   .align 2                            ");
__asm__("   ubc_ubbr: .long 0xffff8608          ");
