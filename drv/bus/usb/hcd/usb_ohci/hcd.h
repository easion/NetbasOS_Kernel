...
///#define spin_lock_irqsave spinlock_cli
//#define spin_unlock_irqrestore spinunlock_restore
...
struct usb_hcd {	/* usb_bus.hcpriv points to this */
	...
	ktimer_t	rh_timer;	/* drives root hub */
	void* rh_timer_data;
	...
	PCI_Info_s		pdev;		/* pci is typical */
...
struct hc_driver {
	...
	/* irq handler */
	void	(*irq) (struct usb_hcd *hcd, SysCallRegs_s* regs);
...
// FIXME: rework generic-to-specific HCD linkage (specific contains generic)
	...
	int	(*urb_enqueue) (struct usb_hcd *hcd, USB_packet_s *urb,
					int mem_flags);
	int	(*urb_dequeue) (struct usb_hcd *hcd, USB_packet_s *urb);
...
extern void usb_claim_bandwidth (USB_device_s *dev, USB_packet_s *urb,
		int bustime, int isoc);
extern void usb_release_bandwidth (USB_device_s *dev, USB_packet_s *urb,
		int isoc);
...
extern int usb_check_bandwidth (USB_device_s *dev, USB_packet_s *urb);
...
extern long hcd_calc_bus_time (int speed, int is_input,
...
static inline USB_bus_driver_s *hcd_to_bus (struct usb_hcd *hcd)
	{ return hcd->bus; }

static inline void
usb_hub_tt_clear_buffer (USB_device_s *dev, int pipe)
	{ }
