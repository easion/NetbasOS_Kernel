...
static SpinLock_s usb_ed_lock = { SPINLOCK_UNLOCKED, }; // INIT_SPIN_LOCK( "usb_ed_lock" );
  			...
  			if (ed->state != ED_NEW) {
  				if (ed->state == ED_OPER) {
					...
					kerndbg( KERN_DEBUG, "driver usb-%i dev %d ed 0x%x unfreed URB\n",
						ohci->ohci_dev->deviceid, usb_dev->nDeviceNum, i);
					ep_unlink (ohci, ed);
				}
  				ep_rm_ed (usb_dev, ed);
  				ed->state = ED_DEL;
...
static int rh_init_int_timer (USB_packet_s * urb) 
{
	...
	init_timer( ohci->rh.rh_int_timer, rh_int_timer_do, (void*)urb);
	install_timer( ohci->rh.rh_int_timer,1000);
	//	(1000 * (urb->nInterval < 30? 30: urb->nInterval)),		 );
	
	return 0;
}

static int rh_restart_int_timer (USB_packet_s * urb) 
{
	ohci_t * ohci = urb->psDevice->psBus->pHCPrivate;
	
	restart_timer( ohci->rh.rh_int_timer, 
		1000 * (urb->nInterval < 30? 30: urb->nInterval));	
	return 0;
}
...
#define TEST_OHCI_OK(x) 			len = (x); break
...
static int rh_submit_urb (USB_packet_s * urb)
{
	...
	*/
		...
		case RH_GET_STATUS: 				 		
				*(__u16 *) data_buf = cpu_to_le16 (1); TEST_OHCI_OK (2);
		case RH_GET_STATUS | RH_INTERFACE: 	 		
				*(__u16 *) data_buf = cpu_to_le16 (0); TEST_OHCI_OK (2);
		...
		case RH_GET_STATUS | RH_OTHER | RH_CLASS: 	
				*(__u32 *) data_buf = cpu_to_le32 (RD_RH_PORTSTAT); TEST_OHCI_OK (4);

		case RH_CLEAR_FEATURE | RH_ENDPOINT:  
			switch (wValue) {
				case (RH_ENDPOINT_STALL): TEST_OHCI_OK (0);
			}
			break;
		...
		case RH_CLEAR_FEATURE | RH_CLASS:
			switch (wValue) {
				...
				case (RH_C_HUB_OVER_CURRENT): 
						WR_RH_STAT(RH_HS_OCIC); TEST_OHCI_OK (0);
			}
			break;
		...
		case RH_CLEAR_FEATURE | RH_OTHER | RH_CLASS:
			switch (wValue) {
				case (RH_PORT_ENABLE): 			
						WR_RH_PORTSTAT (RH_PS_CCS ); TEST_OHCI_OK (0);
				case (RH_PORT_SUSPEND):			
						WR_RH_PORTSTAT (RH_PS_POCI); TEST_OHCI_OK (0);
				case (RH_PORT_POWER):			
						WR_RH_PORTSTAT (RH_PS_LSDA); TEST_OHCI_OK (0);
				case (RH_C_PORT_CONNECTION):	
						WR_RH_PORTSTAT (RH_PS_CSC ); TEST_OHCI_OK (0);
				case (RH_C_PORT_ENABLE):		
						WR_RH_PORTSTAT (RH_PS_PESC); TEST_OHCI_OK (0);
				case (RH_C_PORT_SUSPEND):		
						WR_RH_PORTSTAT (RH_PS_PSSC); TEST_OHCI_OK (0);
				case (RH_C_PORT_OVER_CURRENT):	
						WR_RH_PORTSTAT (RH_PS_OCIC); TEST_OHCI_OK (0);
				case (RH_C_PORT_RESET):			
						WR_RH_PORTSTAT (RH_PS_PRSC); TEST_OHCI_OK (0); 
			}
			break;
		...
		case RH_SET_FEATURE | RH_OTHER | RH_CLASS:
			switch (wValue) {
				case (RH_PORT_SUSPEND):			
						WR_RH_PORTSTAT (RH_PS_PSS ); TEST_OHCI_OK (0); 
				case (RH_PORT_RESET): /* BUG IN HUP CODE *********/
						if (RD_RH_PORTSTAT & RH_PS_CCS)
						    WR_RH_PORTSTAT (RH_PS_PRS);
						TEST_OHCI_OK (0);
				case (RH_PORT_POWER):			
						WR_RH_PORTSTAT (RH_PS_PPS ); TEST_OHCI_OK (0); 
		...
		case RH_SET_ADDRESS: ohci->rh.devnum = wValue; TEST_OHCI_OK(0);
				...
				case (0x01): /* device descriptor */
					len = min(
						  ...
						  min(
						      ...
						      wLength));
					data_buf = root_hub_dev_des; TEST_OHCI_OK(len);
				case (0x02): /* configuration descriptor */
					len = min(
						  ...
						  min(
						      ...
						      wLength));
					data_buf = root_hub_config_des; TEST_OHCI_OK(len);
		...
		case RH_GET_CONFIGURATION: 	*(__u8 *) data_buf = 0x01; TEST_OHCI_OK (1);

		case RH_SET_CONFIGURATION: 	WR_RH_STAT (0x10000); TEST_OHCI_OK (0);
...
static ohci_t * hc_alloc_ohci (pci_state_t* dev, void * mem_base)
{
	ohci_t * ohci;
	void *real;
...
static int
hc_found_ohci(pci_state_t *dev , int irq,
	void *mem_base)
{
	ohci_t * ohci;
	char buf[8], *bufp = buf;
	...
	bufp = __irq_itoa(irq);
#endif
	printk( "OHCI: USB OHCI at membase 0x%lx, IRQ %s\n",
...
static uint32 get_pci_memory_size(const pci_state_t* *pcPCIInfo, int nResource)
{
	uint32 nSize, nBase;
	int nBus = pcPCIInfo->nBus;
	int nDev = pcPCIInfo->deviceid;
...
/* always called with process context; sleeping is TEST_OHCI_OK */

static int
ohci_pci_probe (pci_state_t *dev)
{
	int status;
	void *base;
	uint32 temp;
	...
	g_psPCIBus->read_pci_config16(dev, PCI_COMMAND, &cmd );	
	...
	*/
	...
	base = dev->base[i];
	//remap_area( create_area( "usb_ohci", &base, len + PAGE_SIZE, len + PAGE_SIZE, 
	//AREA_ANY_ADDRESS|AREA_KERNEL,
		//						AREA_FULL_LOCK ), (void*)( resource & PAGE_MASK ) ); 
	//temp = (uint32)base + ( resource - ( resource & PAGE_MASK ) );
	//base = (void*)temp;

	kprintf("hc_found_ohci\n");	

	status = hc_found_ohci (dev, dev->irq, base);
	if (status < 0) {
...
static pci_state_t * usb_dev_probe()
{
    ...
    for (i=0; i<7; i++)
	 if ((usb_device = g_psPCIBus->pci_lookup(usb_lk[i].start, 
		usb_lk[i].vendor_id, usb_lk[i].dev_id)) != NULL ){
		printk("usb_dev_probe: %s\n", usb_lk[i].usb_name);
	    break;
	}
...
status_t device_init( )
{
	...
	if( g_psBus == NULL )
	{
		printk("usb bus_hooks not setup!\n");
		return( -1 );
	}
	...
	if (!pci)
	{
		printk("your pc has not usb devices\n");
	}
