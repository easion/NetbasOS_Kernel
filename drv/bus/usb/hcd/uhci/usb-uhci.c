 ...
 *
 * $Id: usb-uhci.c,v 1.6 2006/08/04 16:59:20 aklenke Exp $
 */
...
#define VERSTR "$Revision: 1.6 $ time " __TIME__ " " __DATE__
...
static int rh_restart_int_timer (USB_packet_s *urb)
{
	...
	restart_timer( uhci->rh.rh_int_timer, 20 * 1000 );
	return 0;
}

/*-------------------------------------------------------------------------*/
#define TEST_UHCI_OK(x) 			len = (x); break
...
static int rh_submit_urb (USB_packet_s *urb)
{
	...
	case RH_CLEAR_FEATURE | RH_CLASS:
		...
		case (RH_C_HUB_OVER_CURRENT):
			TEST_UHCI_OK (0);	/* hub power over current ** */
		}
		break;
...
/*** WR_RH_PORTSTAT(RH_PS_PSSC); */
		...
		case (RH_C_PORT_OVER_CURRENT):
			TEST_UHCI_OK (0);	/* port power over current ** */
	...
	case RH_GET_DESCRIPTOR:
		...
		case (0x03):	/* string descriptors */
			len = g_psBus->root_hub_string (wValue & 0xff,
				...
				data, wLength);
			if (len > 0) {
				TEST_UHCI_OK(min_t(int, leni, len));
			} else 
				stat = -EPIPE;
		}
		break;
...
static int process_transfer (uhci_t *s, USB_packet_s *urb, int mode)
{
  ...
  is_error:
		...
		if ( (actual_length < maxlength)) {
			...
			// all other cases: short read is TEST_UHCI_OK
...
static int alloc_uhci (  PCI_Info_s dev, int irq, unsigned int io_addr, unsigned int io_size)
{
	uhci_t *s;
...
status_t device_suspend( int nDeviceID, int nDeviceHandle, void* pData )
{
	...
	g_psPCIBus->write_pci_config16( s->uhci_pci, USBLEGSUP, 0 );
...
status_t device_resume( int nDeviceID, int nDeviceHandle, void* pData )
{
	...
	g_psPCIBus->write_pci_config16( s->uhci_pci, USBLEGSUP,  USBLEGSUP_DEFAULT );
...
static int uhci_pci_probe (PCI_Info_s dev )
{
	int i;
