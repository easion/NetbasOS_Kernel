...
status_t scsi_request_sense( SCSI_device_s * psDevice, SCSI_sense_s * psSense )
{
	...
	DEBUG_PRINT( "Requesting sense data...\n" );

	if( psSense == NULL )
	{
		DEBUG_PRINT( "scsi_request_sense() called with psSense = NULL\n" );
		return -EINVAL;
	}
	...
	{
		DEBUG_PRINT( "Could not get SCSI sense data. Sense key is 0x%04x\n", ( sCmd.s.nSense[2] & 0xf0 ) );
		return -EIO;
	}
...
status_t scsi_check_sense( SCSI_device_s * psDevice, SCSI_sense_s * psSense, bool bQuiet )
{
	...
	DEBUG_PRINT( "Key 0x%04x Asc 0x%04x Ascq 0x%04x\n", psSense->sense_key, psSense->asc, psSense->ascq );

	switch ( psSense->sense_key )
	{
		case SCSI_NO_SENSE:
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_NO_SENSE\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_RECOVERED_ERROR\n" );
		...
		case SCSI_NOT_READY:
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_NOT_READY\n" );
		...
		{
			if( !bQuiet )
				DEBUG_PRINT( "SCSI drive has reported SCSI_MEDIUM_ERROR\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_HARDWARE_ERROR\n" );
		...
		{
			if( !bQuiet )
				DEBUG_PRINT( "SCSI drive has reported SCSI_ILLEGAL_REQUEST\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_UNIT_ATTENTION\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_DATA_PROTECT\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_BLANK_CHECK\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_VENDOR_SPECIFIC\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_COPY_ABORTED\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_ABORTED_COMMAND\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_VOLUME_OVERFLOW\n" );
		...
		{
			DEBUG_PRINT( "SCSI drive has reported SCSI_MISCOMPARE\n" );
		...
		default:
		{
			DEBUG_PRINT( "SCSI drive is reporting an undefined error\n" );
...
status_t scsi_unit_not_ready( SCSI_device_s * psDevice, SCSI_sense_s * psSense )
{
	...
	switch ( psSense->asc )
	{
		...
		{
			DEBUG_PRINT( "With additional code SCSI_NO_ASC_DATA\n" );
		...
		{
			DEBUG_PRINT( "With additional code SCSI_LOGICAL_UNIT_NOT_READY\n" );
			...
			switch ( psSense->ascq )
			{
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_NOT_REPORTABLE\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_BECOMING_READY\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_MUST_INITIALIZE\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_MANUAL_INTERVENTION\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_FORMAT_IN_PROGRESS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_REBUILD_IN_PROGRESS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_RECALC_IN_PROGRESS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_OP_IN_PROGRESS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_LONG_WRITE_IN_PROGRESS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_SELF_TEST_IN_PROGRESS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_ASSYM_ACCESS_STATE_TRANS\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_TARGET_PORT_STANDBY\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_TARGET_PORT_UNAVAILABLE\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_AUX_MEM_UNAVAILABLE\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_NOTIFY_REQUIRED\n" );
				...
				default:
				{
					DEBUG_PRINT( "and an unknown qualifier 0x%04x\n", psSense->ascq );
		...
		{
			DEBUG_PRINT( "With additional code SCSI_NOT_RESPONDING\n" );
		...
		case SCSI_MEDIUM:
		{
			DEBUG_PRINT( "With additional code SCSI_MEDIUM\n" );
			...
			switch ( psSense->ascq )
			{
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_MEDIUM_TRAY_OPEN\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_MEDIUM_NOT_PRESENT\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_MEDIUM_TRAY_CLOSED\n" );
				...
				{
					DEBUG_PRINT( "and a qualifier of SCSI_MEDIUM_LOADABLE\n" );
		...
		default:
		{
			DEBUG_PRINT( "With an unknown or unsupported additional code 0x%04x\n", psSense->asc );
...
retry:
	...
	if( sCmd.s.sSense.sense_key != SCSI_NO_SENSE || sCmd.nResult != 0 )
	{
		...
		if( nError < 0 )
		{
			DEBUG_PRINT( "Unable to request sense data from SCSI device, aborting.\n" );
			return -EIO;
		}
		...
		switch ( nError )
		{
			...
			case SENSE_FATAL:
			{
				DEBUG_PRINT( "SCSI device reporting fatal error, aborting.\n" );
				return -EIO;
			}
		}
	}

	DEBUG_PRINT( "scsi_test_ready(): Device ready\n" );
...
status_t scsi_read_capacity( SCSI_device_s * psDevice, unsigned long *pnCapacity )
{
	...
	DEBUG_PRINT( "scsi_read_capacity(): %d bytes\n", psDevice->nSize );
...
static status_t scsi_do_start_stop( SCSI_device_s * psDevice, int nFlags )
{
	...
	{
		DEBUG_PRINT( "Error: Could not eject drive. Sense key is %x\n", sCmd.s.sSense.sense_key );
		nError = -EIO;
	}
