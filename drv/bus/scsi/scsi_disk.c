...
static status_t _scsi_disk_ioctl( void *pNode, void *pCookie, uint32 nCommand, void *pArgs, bool bFromKernel )
{
	int nError = 0;
	SCSI_device_s *psDevice = g_ScsiDisk;
...
static int sys_scsi_disk__read(unsigned minor, off_t  pos, char * buf,int count)
{
	int ret;

	ret= scsi_generic_read(g_ScsiDisk,NULL,pos, buf, count) ;
...
static int sys_scsi_disk__write(unsigned minor, off_t  pos, char * buf,int count)
{
   return 	scsi_generic_write(g_ScsiDisk,NULL,pos, buf, count) ;
}

/*
**
*/
static int sys_scsi_disk_ioctl(int dev, int cmd, void *args, int from)
{
	return _scsi_disk_ioctl(g_ScsiDisk,NULL,cmd,args,from );
}


static int sys_scsi_disk_open(const char *f, int mode)
{
	int err = scsi_generic_open(g_ScsiDisk,0,NULL);
	if (err)
	{
		return err;
	}
...
static int sys_scsi_disk_close(int  file)
{
	return 0;
}
...
{
	d_name:		"sd",
	d_author:	" Arno Klenke",
	d_version:	" 1.2 $",
	d_index:	0x300,
	d_kver:	CURRENT_KERNEL_VERSION,
	open:		sys_scsi_disk_open,
	close:		sys_scsi_disk_close,
	read:		sys_scsi_disk__read,
	write:		sys_scsi_disk__write,
	ioctl:		sys_scsi_disk_ioctl,
};
...
static int scsi_disk_decode_partitions( SCSI_device_s * psInode )
{
	...
	for( i = 0; i < nNumPartitions; ++i )
	{
		...
		/*psPartition->nNodeHandle =
		create_device_node( psInode->psHost->get_device_id(), psPartition->nDeviceHandle,
			zNodePath, &g_sDiskOperations, psPartition );*/
...
SCSI_device_s * scsi_add_disk( SCSI_host_s * psHost, int nChannel, int nDevice, int nLun, unsigned char nSCSIResult[] )
{
	...
	/*nError = create_device_node( psHost->get_device_id(),
	psDevice->nDeviceHandle, zNodePath, &g_sDiskOperations, psDevice );
	*/
#endif
