...
SCSI_device_s *g_ScsiDisk = NULL;
SCSI_device_s *g_ScsiCdrom = NULL;
...
SCSI_device_s *scsi_scan_device( SCSI_host_s * psHost, int nChannel, int nDevice, int nLun, int *pnSCSILevel )
{
	...
	switch( ( nSCSIResult[0] & 0x1f ) )
	{
		case SCSI_TYPE_DISK:
		{
			...
			g_ScsiDisk = psDevice = scsi_add_disk( psHost, nChannel, nDevice, nLun, nSCSIResult );
			break;
		}
		...
		case SCSI_TYPE_ROM:
		{
			...
			g_ScsiCdrom = psDevice = scsi_add_cdrom( psHost, nChannel, nDevice, nLun, nSCSIResult );
			break;
		}
...
static struct bus_man scsiBus =
{
	bus_name: "SCSI",
	bus_hooks:scsi_bus_get_hooks,
};
...
int dll_version()
{
	printk("JICAMA SCSI Driver Version 0.01!\n");
	return 0;
}

int dll_destroy()
{
	busman_unregister( &scsiBus );
	return 0;
}
