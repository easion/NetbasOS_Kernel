...
static status_t scsi_cdrom_ioctl( void *pNode, void *pCookie, uint32 nCommand, void *pArgs,int len, bool bFromKernel )
{
...
#define CDROM_DEV_NO 0X3300

static int sys_scsi_cdrom__read(unsigned minor, off_t  pos, char * buf,int count)
{
	int ret;

	ret= scsi_generic_read(g_ScsiCdrom,NULL,pos, buf, count) ;
...
static int sys_scsi_cdrom__write(unsigned minor, off_t  pos, char * buf,int count)
{
   return 	scsi_generic_write(g_ScsiCdrom,NULL,pos, buf, count) ;
}

/*
**
*/
static int sys_scsi_cdrom_ioctl(int dev, int cmd, void *args, int from)
{
	return scsi_cdrom_ioctl(g_ScsiCdrom,NULL,cmd,args,from );
}


static int sys_scsi_cdrom_open(const char *f, int mode)
{
	int err = scsi_generic_open(g_ScsiCdrom,0,NULL);
	if (err)
	{
		return err;
	}
...
static int sys_scsi_cdrom_close(int  file)
{
	return 0;
}
...
static const driver_ops_t ops =
{
	...
	d_version:	" 1.2 $",
	d_index:	CDROM_DEV_NO,
	d_kver:	CURRENT_KERNEL_VERSION,
	open:		sys_scsi_cdrom_open,
	close:		sys_scsi_cdrom_close,
	read:		sys_scsi_cdrom__read,
	write:		NULL,
	ioctl:		sys_scsi_cdrom_ioctl,
};
