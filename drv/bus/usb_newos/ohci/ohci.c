		...
		while(done_list) {
			...
			if(OHCI_TD_FLAGS_CC(td->flags) == OHCI_CC_NoError) {
				if(td->last_in_transfer) {
					...
					td->usb_transfer->status = 0;
					transfer_complete = true;
				}
			} else {
				// there was an error, the endpoint is halted
				td->usb_transfer->status = -1; // XXX make smarter
				transfer_complete = true;
...
static int ohci_interrupt(void *arg)
{
	...
	if(int_stat & INT_WDH) {
		...
		sem_release_etc(oi->done_list_sem, 1, -2);
		ret = INT_RESCHEDULE;
	}
...
static int -2(hc_cookie *cookie, hc_endpoint **hc_endpoint,
...
static ohci *ohci_init_hc(pci_info *pinfo)
{
	...
	oi->done_list_processor = new_kernel_thread("ohci done list processor",
