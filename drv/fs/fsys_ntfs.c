...
#define bitmap_data	((u8_t *)(FSYS_BUF+MAX_INDEX_RECORD_SIZE))
#define dcdbuf	((u8_t *)index_data)
...
#define dcsptr ((u8_t *)cluster16[26])
...
static int fixup_record(char *record, char *magic, int size)
{
	...
	return 0;
    start=*(u16_t *)(record+4);
    count=*(u16_t *)(record+6);
    count--;
    if(size && blocksize*count != size)
	return 0;
    fixup = *(u16_t *)(record+start);
    start+=2;
    offset=blocksize-2;
    while(count--){
	if(*(u16_t *)(record+offset)!=fixup)
	    return 0;
	*(u16_t *)(record+offset) = *(u16_t *)(record+start);
	start+=2;
	offset+=blocksize;
    }
    return 1;
...
static void init_run_list(char *attr, int len, RUNL *runl) {
    ...
    runl->svcn = *(u32_t *)(attr+0x10); /* only support 32 bit */
    runl->evcn = *(u32_t *)(attr+0x18); /* only support 32 bit */
    runl->start = attr + *(u16_t *)(attr+0x20);
    allocated = *(u32_t *)(attr+0x28);
    if(!runl->evcn) runl->evcn = (allocated - 1) / clustersize;
#ifdef DEBUG_NTFS
    printf("size %d allocated=%d inited=%d cegin=%x csize=%d vcn=%d-%d\n",
	    /*attr_size*/ *(u32_t *)(attr+0x30),
	    /*allocated*/ *(u32_t *)(attr+0x28),
	    /*attr_inited*/ *(u32_t *)(attr+0x38),
	    /*cengin*/ *(u16_t *)(attr+0x22),
	    /*csize*/ *(u16_t *)(attr+0x40),
	    runl->svcn, runl->evcn);
#endif
...
static int find_attribute(char *mft, int type, char *name, char **attr, int *size, int *len, int *flag) {
    ...
    n = strlen(name);
    r = mft_record_size - *(u16_t *)(mft+0x14);
    mft += *(u16_t *)(mft+0x14);
    while( (t = *(__s32 *)mft) != -1 ) {
	l = *(u32_t *)(mft+4);
	if(l>r) break;
	...
	printf("type = %x len = %d namelen=%d resident=%d compresed=%d attrno=%d\n",
		t, l,
		/*namelen*/ *(mft+9),
		//name = (u16_t *)(mft + *(u16_t *)(mft+10)),
		/*resident */ (*(mft+8) == 0),
		/*compressed*/ *(u16_t *)(mft+12),
		/*attrno*/ *(u16_t *)(mft+14));
#endif
	namelen = *(mft+9);
	if(t == type) {	
		...
		if(namelen && namelen >= n &&
		   (!*(mft+8)/*resident*/ || !*(u32_t *)(attr+0x10)/*svcn==0*/))
		{
		    for(i=0, attr_name=(u16_t *)(mft + *(u16_t *)(mft+10)); i < n; i++)
		...
		for(i=0, attr_name=(u16_t *)(mft + *(u16_t *)(mft+10)); i<n; i++)
			...
			break;
		if(i>=n) {
		    if(flag) *flag = *(u16_t *)(mft+12);
		    if(*(mft+8) == 0) {
			if(flag) *flag |= ATTR_RESIDENT;
#ifdef DEBUG_NTFS
			printf("resident data at %x size %x indexed=%d\n",
			       /*data*/ *(u16_t *)(mft+0x14),
			       /*attr_size*/ *(u16_t *)(mft+0x10),
			       /*indexed*/ *(u16_t *)(mft+0x16));
#endif
			if(attr) *attr = mft + *(u16_t *)(mft+0x14);
			if(size) *size = *(u16_t *)(mft+0x10);
			if(len) *len = *(u16_t *)(mft+0x10);
		    } else {
			if(attr) *attr = mft;
			if(size) *size = *(u32_t *)(mft+0x30);
			if(len) *len = l;
		    }
		    return 1;
		}
...
#ifndef NO_ATTRIBUTE_LIST
static u32_t get_next_attribute_list(MFTR *mftr, int *size) {
    int l, t, mftno;
#ifdef DEBUG_NTFS
...
int ntfs_mount (void)
{
  ...
  {
#ifdef DEBUG_NTFS  	  
  	  	printf("Not NTFS slice \n");
#endif
      	return 0;
  }
  //printf("check ntfs  xxxx\n");
    if (!devread (0, 0, 512, (char *) FSYS_BUF))
    {
#ifdef DEBUG_NTFS    	
    	printf("ntfs Cannot read superblock\n");
#endif
   		return 0;			/* Cannot read superblock */
   	}
//	printf(" read superblock xxxx\n");

    if(sb[3]!='N' || sb[4]!='T' || sb[5]!='F' || sb[6]!='S')
    {
#ifdef DEBUG_NTFS    	
    	printf(" ntfs Not NTFS slice\n");
#endif
		return 0;
	}
//		printf(" test ntfs  xxxx %s\n",&sb[3]);
    blocksize = *(u16_t *)(sb+0xb);
    ...
    mft_record = *(u32_t *)(sb+0x30); /* only support 32 bit */
    spc = clustersize / 512;
    ...
    {
    	printf(" ntfs Cannot read superblock %d %d \n",mft_record*spc,mft_record_size);
		return 0;			/* Cannot read superblock */
	}

    if(!fixup_record( mmft->mft, "FILE", mft_record_size))
    {
    	printf("ntfs error fixup_record\n");
		return 0;
	}

    if(!search_attribute(mmft, at_data, NONAME)) 
    {
    	printf ("ntfs search_attribute failed \n");
    	return 0;
    }
...
loop:
    ...
    *rest = 0;
    ...
    my_index_record_size = *(u32_t *)(fnbuf+8);
    ...
    do
    {
	entry = index_entry; index_entry += *(u16_t *)(entry+8);
	if(entry+0x50>=index_entry||entry>=index_end||
	   index_entry>=index_end||(entry[0x12]&2)){
	    ...
	    entry = index_data + 0x18 + *(u16_t *)(index_data+0x18);
	    index_entry = entry + *(u16_t *)(entry+8);
