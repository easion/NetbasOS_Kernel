...
class Inode : public CachedBlock {
	public:
		...
		static status_t Create(Transaction *transaction, Inode *parent, const char *name,
					int32 mode, int omode, uint32 type, off_t *_id = NULL, Inode **_inode = NULL, bool bNoInodeAccess=false);
...
CachedBlock::Unset()
{
	if (fBlock != NULL)
	{
		release_cache_block(fVolume->Device(), fBlockNumber);
	}
...
//						  : (uint8 *)get_block(fVolume->Device(), block, BlockSize());
	if( empty )
	{
		fBlock = (uint8 *)get_empty_block(fVolume->Device(), block, BlockSize());
		memset( fBlock, 0, BlockSize() );
		return( fBlock );
	} else {
		return( fBlock = (uint8 *)get_cache_block(fVolume->Device(), block, BlockSize()) );
	}
}
