...
class DeviceOpener {
	public:
...
#else
		DeviceOpener(kdev_t devnum, const Devfd_t device, int mode);
		Devfd_t Open(const Devfd_t device, int mode);
#endif
		~DeviceOpener();
		...
		Devfd_t Device() const { return fDevice; }
...
DeviceOpener::DeviceOpener(kdev_t devnum, 
#ifdef __USER_MODE__
const char *device,
#else
const Devfd_t device,
#endif
int mode)
	:
...
{
	fDevice = open(device, mode); //dpp
	
	return fDevice;
}
#else
Devfd_t 
DeviceOpener::Open(const Devfd_t device, int mode)
{
	fDevice = device; //dpp
...
disk_super_block::IsValid()
{
	if (Magic1() != (int32)SUPER_BLOCK_MAGIC1
		...
		|| NumBlocks() < 10
		|| AllocationGroups() != divide_roundup((int32)NumBlocks(),
			(int32)(1L << AllocationGroupShift())))
		return false;
...
#else
status_t
Volume::Mount(const Devfd_t deviceName, uint32 flags)
#endif
{
...
status_t
Volume::Identify(Devfd_t fd, disk_super_block *superBlock)
{
	char buffer[1024];
