...
BufferPool::~BufferPool()
{
	destroy_semaphore(fFreeBuffers);

	lock_semaphore(fLock);
...
BufferPool::RequestBuffers(uint32 blockSize)
{
	...
	status_t status = lock_semaphore(fLock);
	if (status == B_OK) {
		*(buffers[0]) = fFirstFree;
		fFirstFree = buffers[kNumBuffers - 1];
		unlock_semaphore(fLock);
...
{
	status_t status = unlock_semaphore_ex(fFreeBuffers, kNumBuffers); //fixme
	if (status < B_OK)
		return status;

	status = lock_semaphore(fLock);
	if (status < B_OK)
		return status;
...
{
	status_t status = lock_semaphore(fFreeBuffers);
	if (status < B_OK)
		return status;

	if ((status = lock_semaphore(fLock)) < B_OK) {
		unlock_semaphore(fFreeBuffers);
		return status;
	}
