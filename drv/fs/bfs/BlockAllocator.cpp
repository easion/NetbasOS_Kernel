...
BlockAllocator::Initialize(bool full)
{
	...
	pid_t id = new_kernel_thread("bfs block allocator", (thread_func)BlockAllocator::initialize,
			(void *)this);
	if (id < B_OK)
		return initialize(this);

	return 0;//wakeup_thread(id, true);
}
...
#if 0 /*def __SYLLABLE__*/
	off_t offset = allocator->fVolume->Bitmap();
	printk("Offset %i\n", (int)offset );
#else
	off_t offset = 1;
#endif
...
BlockAllocator::CheckNextNode(check_control *control)
{
	...
	while (true) {
		...
		status_t status = cookie->iterator->GetNextEntry((void*)name, &length, B_FILE_NAME_LENGTH, (off_t *)&id);
