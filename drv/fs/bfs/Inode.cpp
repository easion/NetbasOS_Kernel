...
InodeAllocator::New(block_run *parentRun, mode_t mode, block_run &run, Inode **_inode)
{
	...
	node->create_time = HOST_ENDIAN_TO_BFS_INT64((bigtime_t)( get_unix_time()  ) << INODE_TIME_SHIFT);
...
Inode::RemoveAttribute(Transaction *transaction, const char *name)
{
	...
	if (status == B_OK) {
		...
		if (attributes->IsEmpty()) {
			...
			set_inode_deleted_flag(fVolume->ID(), attributes->ID(), true);
			if (/*remove_vnode(fVolume->ID(), attributes->ID())*/B_OK == B_OK) {
				...
				if (attributes->WriteBack(transaction) == B_OK) {
					...
					set_inode_deleted_flag(fVolume->ID(), attributes->ID(), false);
...
Inode::GetAttribute(const char *name, Inode **attribute)
{
	...
	if (status == B_OK) {
		vnode_id id;
		if ((status = tree->Find((uint8 *)name, (uint16)strlen(name), (off_t*)&id)) == B_OK) {
...
Inode::IsEmpty()
{
	...
	uint16 length;
	vnode_id id;
	while (iterator.GetNextEntry(name, &length, B_FILE_NAME_LENGTH, (off_t*)&id) == B_OK) {
...
Inode::WriteAt(Transaction *transaction, off_t pos, const uint8 *buffer, size_t *_length)
{
	...
	Node()->last_modified_time = (bigtime_t)( get_unix_time()  ) << INODE_TIME_SHIFT;
...
Inode::Sync()
{
	...
	for (int32 i = 0; i < NUM_DIRECT_BLOCKS; i++) {
		...
		status = flush_cache_block(fVolume->Device(), fVolume->ToBlock(data->direct[i])/*,
	...
	for (int32 j = 0; j < data->indirect.Length(); j++) {
		...
		for (int32 i = 0; i < count; i++) {
			...
			status = flush_cache_block(fVolume->Device(), fVolume->ToBlock(runs[i])/*, runs[i].Length()*/);
			if (status != B_OK)
				return status;
		}
	}
		...
		for (int32 k = 0; k < count; k++) {
			...
			for (int32 j = 0; j < indirectRuns[k].Length(); j++) {
				...
				for (int32 i = 0; i < count; i++) {
					...
					// they are adjacent
					status = flush_cache_block(fVolume->Device(), fVolume->ToBlock(runs[i])/*,
						runs[i].Length()*/);
					if (status != B_OK)
						return status;
				}
...
Inode::Remove(Transaction *transaction, const char *name, off_t *_id, bool isDirectory)
{
	...
	//	return B_ERROR;
	set_inode_deleted_flag(fVolume->ID(), id, true);
	if (tree->Remove(transaction, name, id) < B_OK) {
		//unremove_vnode(fVolume->ID(), id);
		set_inode_deleted_flag(fVolume->ID(), id, false);
		RETURN_ERROR(B_ERROR);
	}
#ifdef DEBUG
	if (tree->Find((uint8 *)name, (uint16)strlen(name), &id) == B_OK) {
...
Inode::Create(Transaction *transaction, Inode *parent, const char *name, int32 mode,
	int omode, uint32 type, off_t *_id, Inode **_inode, bool bNoInodeAccess)
{
	FUNCTION();
	...
	if( bNoInodeAccess == false )
	{
		ino_t id = inode->ID();
		delete( inode );
		if ((status = get_vnode(volume->ID(), id, (void**)&inode)) != B_OK) {
			// this is a really fatal error, and we can't recover from that
			FATAL(("inode.c: new_vnode() failed with: %s\n", strerror(status)));
			DIE(("inode.c: new_vnode() failed for inode!"));
		}
	}
...
AttributeIterator::GetNext(char *name, size_t *_length, uint32 *_type, vnode_id *_id)
{
	...
	uint16 length;
	vnode_id id;
	status_t status = fIterator->GetNextEntry(name, (uint16*)&length, B_FILE_NAME_LENGTH, (off_t*)&id);
	if (status < B_OK)
		return status;
