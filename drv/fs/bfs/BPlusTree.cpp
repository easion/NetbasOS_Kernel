...
CachedNode::Unset()
{
	...
	if (fBlock != NULL) {
		release_cache_block(fTree->fStream->GetVolume()->Device(), fBlockNumber);
...
CachedNode::SetTo(off_t offset, bool check)
{
	...
	if (InternalSetTo(offset) != NULL && check) {
		...
		if (!header->IsValidLink(fNode->LeftLink())
			...
			|| (int8 *)((int8 *)fNode->Values() + fNode->NumKeys() * sizeof(off_t)) >
...
CachedNode::InternalSetTo(off_t offset)
{
	...
	if (offset < fTree->fStream->Size()
		...
		fBlock =  (uint8 *)get_cache_block(volume->Device(), fBlockNumber, volume->BlockSize());
		if (fBlock) {
	...
	off_t otherOffset, uint16 *_keyIndex, uint8 *key, uint16 *_keyLength, off_t *_value)
{
	...
	if (node->OverflowLink() != BPLUSTREE_NULL) {
		if (in == keyIndex) {
			...
			newKey = (uint8 *)malloc((size_t)newLength);
			if (newKey == NULL)
				return B_NO_MEMORY;
