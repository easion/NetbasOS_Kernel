...
Index::SetTo(const char *name)
{
	...
	vnode_id id;
	status_t status = tree->Find((uint8 *)name, (uint16)strlen(name), (off_t*)&id);
	if (status != B_OK)
		return status;
...
status_t
Index::Create(Transaction *transaction, const char *name, uint32 type, bool bNoInodeAccess)
{
	Unset();
	fNoInodeAccess = bNoInodeAccess;
	...
	return Inode::Create(transaction, fVolume->IndicesNode(), name,
		S_INDEX_DIR | S_DIRECTORY | mode, 0, type, NULL, bNoInodeAccess ? 0 : &fNode, bNoInodeAccess);
}
...
Index::UpdateLastModified(Transaction *transaction, Inode *inode, off_t modified)
{
	...
	if (modified == -1)
		modified = (bigtime_t)( get_unix_time()  ) << INODE_TIME_SHIFT;
	modified |= fVolume->GetUniqueID() & INODE_TIME_MASK;

	if( !inode->IsDirectory() )
	{
		status = Update(transaction, "last_modified", B_INT64_TYPE, (uint8 *)&oldModified,
