		...
		wchar_t *name, __u32 name_len, int ic, __u16 instance)
{
	...
	for (; a >= m && a <= m + vol->mft_record_size;
		...
		if (instance != NTFS_GETU16(a + 14 /* instance */))
			continue;
		ntfs_debug( "ntfs_find_attr_in_mft_record: found: "
		...
		unsigned char *data)
{
	...
	if (vcn != startvcn) {
		ntfs_debug( "ntfs_process_runs: ino = 0x%x, "
	...
	ntfs_debug( "ntfs_process_runs: startvcn = 0x%x, vcn = 0x%x"
				", endvcn = 0x%x, cnum = %i\n", startvcn, vcn,
				endvcn, cnum);
	return er;
}
...
int ntfs_insert_attribute(ntfs_inode *ino, unsigned char *attrdata)
{
	...
	ntfs_debug( "ntfs_insert_attribute: ino->i_number 0x%x, "
			"attr type 0x%x\n", ino->i_number, type);
	...
	if (NTFS_GETU8(attrdata + 8) == 0)
		...
		error = ntfs_new_attr(ino, type, name, namelen, NULL, 0, &i,
				&found);
	if (error) {
		ntfs_debug( "ntfs_insert_attribute: ntfs_new_attr "
				"failed.\n");
		if (name)
			ntfs_free(name);
		return error;
	...
	if (found) {
		...
		if (!ino->attrs[i].resident) {
			ntfs_debug( "ntfs_insert_attribute:"
						" processing runs 1.\n");
	...
	if (attr->resident) {
		...
		ntfs_debug( "ntfs_insert_attribute: processing "
				"runs 2.\n");
		/* FIXME: Check error code! (AIA) */
