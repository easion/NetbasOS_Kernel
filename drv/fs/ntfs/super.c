...
int ntfs_fixup_record(char *record, char *magic, int size)
{
	...
	if (size & (NTFS_SECTOR_SIZE - 1) || start & 1 ||
		...
		if (size <= 0)
			printk( "NTFS: BUG: ntfs_fixup_record() got "
...
int ntfs_init_volume(ntfs_volume *vol, char *boot)
{
	...
	ntfs_debug( "ntfs_init_volume: vol->sector_size = 0x%x\n",
				vol->sector_size);
	ntfs_debug( "ntfs_init_volume: sectors_per_cluster = "
				"0x%x\n", NTFS_GETU8(boot + 0xD));
	sectors_per_cluster_bits = ffs(NTFS_GETU8(boot + 0xD)) - 1;
	ntfs_debug( "ntfs_init_volume: sectors_per_cluster_bits "
				"= 0x%x\n", sectors_per_cluster_bits); 
	vol->mft_clusters_per_record = NTFS_GETS8(boot + 0x40);
	ntfs_debug( "ntfs_init_volume: vol->mft_clusters_per_record"
				" = 0x%x\n", vol->mft_clusters_per_record); 
	vol->index_clusters_per_record = NTFS_GETS8(boot + 0x44);
	ntfs_debug( "ntfs_init_volume: "
	...
	ntfs_debug( "ntfs_init_volume: vol->cluster_size = 0x%x\n",
				vol->cluster_size); 
	ntfs_debug( "ntfs_init_volume: vol->cluster_size_bits = "
	...
	ntfs_debug( "ntfs_init_volume: vol->mft_record_size = 0x%x"
				"\n", vol->mft_record_size); 
	ntfs_debug( "ntfs_init_volume: vol->mft_record_size_bits = "
	...
	ntfs_debug( "ntfs_init_volume: vol->index_record_size = "
				"0x%x\n", vol->index_record_size);
	ntfs_debug( "ntfs_init_volume: vol->index_record_size_bits "
	...
	ntfs_debug( "ntfs_init_volume: vol->nr_clusters = 0x%x\n",
			vol->nr_clusters);
	...
	ntfs_debug( "ntfs_init_volume: vol->mft_zone_pos = %x\n",
			vol->mft_zone_pos);
	/*
	...
	ntfs_debug( "ntfs_init_volume: vol->mft_zone_start = %x\n",
			vol->mft_zone_start);
	/*
	...
	ntfs_debug( "ntfs_init_volume: vol->mft_zone_end = %x\n",
			vol->mft_zone_end);
	/*
	...
	ntfs_debug( "ntfs_init_volume: vol->data1_zone_pos = %x\n",
			vol->data1_zone_pos);
	vol->data2_zone_pos = (ntfs_cluster_t)0;
	ntfs_debug( "ntfs_init_volume: vol->data2_zone_pos = %x\n",
			vol->data2_zone_pos);
...
static int process_attrdef(ntfs_inode* attrdef, ntfs_u8* def)
{
	...
	if (!type) {
		ntfs_debug( "process_atrdef: finished processing "
							"and returning 1\n");
		return 1;
	}
	...
	ntfs_debug( "process_attrdef: found %s attribute of type "
...
int ntfs_init_attrdef(ntfs_inode* attrdef)
{
	...
	ntfs_debug( "Entered ntfs_init_attrdef()\n");
	...
	ntfs_debug( "In ntfs_init_attrdef() after call to "
			"ntfs_find_attr.\n");
	if (!data) {
		ntfs_free(buf);
		return -EINVAL;
...
int ntfs_insert_fixups(unsigned char *rec, int rec_size)
{
	...
	if (first + count * 2 > NTFS_SECTOR_SIZE - 2) {
		printk( "NTFS: ntfs_insert_fixups() detected corrupt "
		...
		const NTFS_CLUSTER_ALLOCATION_ZONES zone)
{
	...
	ntfs_debug( "%s(): Entering with *location = 0x%x, "
			"*count = 0x%x, zone = %s_ZONE.\n", __FUNCTION__,
			*location, *count, zone == DATA_ZONE ? "DATA" : "MFT");
	buf = (char*)kmalloc(4096,0);
	if (!buf) {
		ntfs_debug( "%s(): Returning -ENOMEM.\n",
				__FUNCTION__);
		return -ENOMEM;
	}
	io.fn_put = ntfs_put;
	...
	rlpos = rlsize = 0;
	if (*count <= 0) {
		ntfs_debug( "%s(): *count <= 0, "
				"returning -EINVAL.\n", __FUNCTION__);
		err = -EINVAL;
		goto err_ret;
	}
	while (1) {
		ntfs_debug( "%s(): Start of outer while "
		...
		buf_pos &= ~7;
		need_writeback = 0;
		ntfs_debug( "%s(): Before inner while "
		...
		while (lcn < buf_size && lcn + buf_pos < zone_end) {
			byte = buf + (lcn >> 3);
			ntfs_debug( "%s(): In inner while loop: "
			...
			*byte |= bit;
			...
			need_writeback = 1;
			ntfs_debug( "%s(): *byte = 0x%x, "
					"need_writeback = %i.\n", __FUNCTION__,
					*byte, need_writeback);
			/* Reallocate memory if necessary. */
			if ((rlpos + 2) * sizeof(ntfs_runlist) >= rlsize) {
				ntfs_debug( "%s(): Reallocating "
						"space.\n", __FUNCTION__);
			 ...
			 */
			ntfs_debug( "%s(): Adding run (lcn 0x%x, "
			...
			if (prev_lcn == lcn + buf_pos - prev_run_len && rlpos) {
				ntfs_debug( "%s(): Coalescing to "
						"run (lcn 0x%x, len 0x%x).\n",
						__FUNCTION__,
						rl2[rlpos - 1].lcn,
						rl2[rlpos - 1].len);
				rl2[rlpos - 1].len = ++prev_run_len;
				ntfs_debug( "%s(): Run now (lcn "
		...
		buf_pos += buf_size;
		ntfs_debug( "%s(): After inner while "
				"loop: buf_size = 0x%x, lcn = 0x%x, buf_pos = "
				"0x%x, need_writeback = %i.\n", __FUNCTION__,
				buf_size, lcn, buf_pos, need_writeback);
		if (need_writeback) {
			ntfs_debug( "%s(): Writing back.\n",
					__FUNCTION__);
			need_writeback = 0;
			io.param = buf;
			io.do_read = 0;
...
zone_pass_done:	/* Finished with the current zone pass. */
		ntfs_debug( "%s(): At zone_pass_done, pass = %i.\n",
				__FUNCTION__, pass);
		if (pass == 1) {
			/*
...
done_zones_check:
		ntfs_debug( "%s(): At done_zones_check, "
		...
		if (done_zones < 7) {
			ntfs_debug( "%s(): Switching zone.\n",
					__FUNCTION__);
			/* Now switch to the next zone we haven't done yet. */
			pass = 1;
			switch (search_zone) {
			case 1:
				ntfs_debug( "%s(): Switching from "
...
switch_to_data1_zone:		search_zone = 2;
			...
			ntfs_debug( "%s(): After zone switch, "
			...
			buf_pos = zone_start;
			if (zone_start == zone_end) {
				ntfs_debug( "%s(): Empty zone, "
						"going to done_zones_check.\n",
						__FUNCTION__);
				/* Empty zone. Don't bother searching it. */
				goto done_zones_check;
			}
			ntfs_debug( "%s(): Continuing outer while "
					"loop.\n", __FUNCTION__);
			continue;
		} /* done_zones == 7 */
		ntfs_debug( "%s(): All zones are finished.\n",
				__FUNCTION__);
		/*
		...
		ntfs_debug( "%s(): vol->mft_zone_start = 0x%x, "
		...
		if (zone == MFT_ZONE || mft_zone_size <= (ntfs_cluster_t)0) {
			ntfs_debug( "%s(): No free clusters left, "
		...
		ntfs_debug( "%s(): Shrinking mft zone.\n",
				__FUNCTION__);
		zone_end = vol->mft_zone_end;
		mft_zone_size >>= 1;
		...
		pass = 2;
		done_zones &= ~2;
		ntfs_debug( "%s(): After shrinking mft "
	...
	ntfs_debug( "%s(): After outer while loop.\n",
			__FUNCTION__);
done_ret:
	ntfs_debug( "%s(): At done_ret.\n", __FUNCTION__);
	...
	*rl_len = rlpos;
	if (need_writeback) {
		...
		ntfs_debug( "%s(): Wrote 0x%Lx bytes.\n",
				__FUNCTION__, io.size);
	}
done_fail_ret:
	ntfs_debug( "%s(): At done_fail_ret (follows done_ret).\n",
			 __FUNCTION__);
	unlock_kernel();
	kfree((unsigned long)buf);
	if (err)
		ntfs_debug( "%s(): Failed to allocate "
				"clusters. Returning with error code %i.\n",
				__FUNCTION__, -err);
	ntfs_debug( "%s(): Syncing $Bitmap inode.\n", __FUNCTION__);
	if (ntfs_update_inode(vol->bitmap))
		ntfs_error("%s(): Failed to sync inode $Bitmap. "
				"Continuing anyway.\n", __FUNCTION__);
	ntfs_debug( "%s(): Returning with code %i.\n", __FUNCTION__,
			err);
	return err;
fail_ret:
	ntfs_debug( "%s(): At fail_ret.\n", __FUNCTION__);
	if (rl2) {
		if (err == -ENOSPC) {
			/* Return first free lcn and count of free clusters. */
			*location = rl2[0].lcn;
			*count -= clusters;
			ntfs_debug( "%s(): err = -ENOSPC, "
					"*location = 0x%x, *count = 0x%x.\n",
					__FUNCTION__, *location, *count);
		}
		/* Deallocate all allocated clusters. */
		ntfs_debug( "%s(): Deallocating allocated "
				"clusters.\n", __FUNCTION__);
		...
		if (err == -ENOSPC) {
			...
			*count = 0;
			ntfs_debug( "%s(): No space left at all, "
	...
	*rl = NULL;
	*rl_len = 0;
	ntfs_debug( "%s(): *rl = NULL, *rl_len = 0, "
			"going to done_fail_ret.\n", __FUNCTION__);
	goto done_fail_ret;
wb_err_ret:
	ntfs_debug( "%s(): At wb_err_ret.\n", __FUNCTION__);
	if (need_writeback) {
		int __err;
		ntfs_debug( "%s(): Writing back.\n", __FUNCTION__);
		io.param = buf;
...
err_ret:
	ntfs_debug( "%s(): At err_ret, *location = -1, "
