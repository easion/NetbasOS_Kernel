...
ntfs_attribute *ntfs_find_attr(ntfs_inode *ino, int type, char *name)
{
	...
	if (!ino) {
		kprintf("ntfs_find_attr: NO INODE!\n");
		return 0;
	}
	for (i = 0; i < ino->attr_count; i++) {
		if (type < ino->attrs[i].type)
			return 0;
		//kprintf("ino->attrs[i].type=%x\n", ino->attrs[i].type);
		if (type == ino->attrs[i].type) {
			if (!name) {
				if (!ino->attrs[i].name)
					return ino->attrs + i;
	...
	//kprintf("%s() not found inode type %x\n",__FUNCTION__, type, ino->attr_count);
	return 0;
}
		...
		ntfs_u8 *attr)
{
	...
	if (NTFS_GETU32(attr) == -1) {
		ntfs_debug( "ntfs_insert_mft_attribute: attribute "
				"type is -1.\n");
		return 0;
	}
...
static void ntfs_load_attributes(ntfs_inode *ino)
{
	...
	ntfs_debug( "load_attributes 0x%x 1\n", 0);
	if (ntfs_insert_mft_attributes(ino, ino->attr, 0))
		return;
	ntfs_debug( "load_attributes 0x%x 2\n", 0);
	//取得属性例表
	alist = ntfs_find_attr(ino, vol->at_attribute_list, 0);
	ntfs_debug( "load_attributes 0x%x 3\n", 0);
	if (!alist)
		return;
	ntfs_debug( "load_attributes 0x%x 4\n", 0);
	datasize = alist->size;
	ntfs_debug( "load_attributes 0x%x: alist->size = 0x%x\n",
			0, alist->size);
	if (alist->resident) {
		//属性分析
	...
	ntfs_debug( "load_attributes 0x%x 5\n", 0);
	...
	for (offset = 0; datasize; datasize -= len, offset += len) {
		...
		if (len > datasize)
			len = datasize;
		ntfs_debug( "load_attributes 0x%x: len = %i\n",
						0, len);
		ntfs_debug( "load_attributes 0x%x: delta = %i\n",
						0, delta);
		io.size = len;
		...
		delta += len;
		ntfs_debug( "load_attributes 0x%x: after += len, "
				"delta = %i\n", 0, delta);
		parse_attributes(ino, buf, &delta);
		ntfs_debug( "load_attributes 0x%x: after "
				"parse_attr, delta = %i\n", 0,
				delta);
		if (delta)
	...
	ntfs_debug( "load_attributes 0x%x 6\n", 0);
	ntfs_free(buf);
}
	
int ntfs_init_inode(ntfs_inode *ino, ntfs_volume *vol, ino_t inum)
{
	char *buf;
	int error;

	ntfs_debug( "Initializing inode 0x%x\n", inum);
	ino->i_number = inum;
	ino->vol = vol;
	ino->attr = buf = ntfs_malloc(vol->mft_record_size);
	if (!buf){
		ntfs_error("%s() no memory\n", __FUNCTION__);
		return -ENOMEM;
	}
	...
	if (error) {
		ntfs_error( "Init inode: 0x%x failed\n", inum);
		return error;
	}
	ntfs_debug( "Init inode: got mft 0x%x\n", inum);
	ino->sequence_number = NTFS_GETU16(buf + 0x10); //序列号码
	ino->attr_count = 0;
	ino->record_count = 0;
	ino->records = 0;
	ino->attrs = 0;
	...
	ntfs_debug( "Init inode: done 0x%x\n", inum);
	return 0;
}
...
//	ntfs_debug( "ntfs_decompress_run: cluster = 0x%x\n",
//								*cluster);
	*data += (type >> 4);
	return 0;
}
		...
		ntfs_io *dest)
{
	...
	ntfs_debug( "%s(): %s 0x%x bytes at offset "
	...
	//写的情况
		...
		if (!attr->resident) {
			/* Has amount of data increased? 数据增多*/
			if (offset + l > attr->size)
				attr->size = offset + l;
	...
	copied = 0;
	while (l) {
		s_vcn = offset >> clustersizebits; //虚拟簇号码
		cluster = attr->d.r.runlist[rnum].lcn;
		len = attr->d.r.runlist[rnum].len;
		s_cluster = cluster + s_vcn - vcn; //逻辑簇号
		   ...
		   ntfs_io *buf)
{
	...
	if (!attr) {
		ntfs_error( "%s(): attr 0x%x not found in node "
		    ...
		    ntfs_io *buf)
{
	...
	attr = ntfs_find_attr(ino, type, name);
	if (!attr) {
		ntfs_debug( "%s(): attr 0x%x not found in inode "
				"0x%x\n", __FUNCTION__, type, 0);
		return -EINVAL;
	}
	return ntfs_readwrite_attr(ino, attr, offset, buf);
...
int ntfs_update_inode(ntfs_inode *ino)
{
	...
	for (i = 0; i < store.count; i++) {
		...
		if (error) {
			printk( "NTFS: ntfs_update_inode() caught "
...
static void dump_runlist(const ntfs_runlist *rl, const int rlen)
{
	...
	ntfs_debug( "%s(): rlen = %i.\n", __FUNCTION__, rlen);
	ntfs_debug( "VCN        LCN        Run length\n");
	for (i = 0, ct = 0; i < rlen; ct += rl[i++].len) {
		if (rl[i].lcn == (ntfs_cluster_t)-1)
			ntfs_debug( "0x%-8x LCN_HOLE   0x%-8x "
					"(%s)\n", ct, rl[i].len, rl[i].len ?
					"sparse run" : "run list end");
		else
			ntfs_debug( "0x%-8x 0x%-8x 0x%-8x%s\n", ct,
		...
		int r2len)
{
	...
	ntfs_debug( "%s(): Entering with *r1len = %i, "
			"r2len = %i.\n", __FUNCTION__, *r1len, r2len);
	ntfs_debug( "%s(): Dumping 1st runlist.\n", __FUNCTION__);
	if (*rl1)
		dump_runlist(*rl1, *r1len);
	else
		ntfs_debug( "%s(): Not present.\n", __FUNCTION__);
	ntfs_debug( "%s(): Dumping 2nd runlist.\n", __FUNCTION__);
	dump_runlist(rl2, r2len);
	rlen = *r1len + r2len + 1;
	rl_size = (rlen * sizeof(ntfs_runlist) + PAGE_SIZE - 1) &
			PAGE_MASK;
	ntfs_debug( "%s(): rlen = %i, rl_size = %i.\n",
	...
	if (rl_size <= ((*r1len * sizeof(ntfs_runlist) + PAGE_SIZE - 1) &
		...
		rl = *rl1;
		ntfs_debug( "%s(): Have enough space already.\n",
				__FUNCTION__);
	} else {
		/* Need more space. Reallocate. */
		ntfs_debug( "%s(): Need more space.\n",
				__FUNCTION__);
	...
	*r1len = rlen;
	ntfs_debug( "%s(): Dumping result runlist.\n",
			__FUNCTION__);
	dump_runlist(*rl1, *r1len);
	ntfs_debug( "%s(): Returning with *r1len = %i.\n",
			__FUNCTION__, rlen);
	return 0;
}
...
static int ntfs_alloc_mft_record(ntfs_volume *vol, unsigned long *result)
{
	...
	*result = -1UL;
	...
	ntfs_debug( "%s(): nr_mft_records = %lu.\n", __FUNCTION__,
			nr_mft_records);
	...
	io.fn_put = ntfs_put;
	io.fn_get = ntfs_get;
	ntfs_debug( "%s(): Starting bitmap search.\n",
			__FUNCTION__);
	ntfs_debug( "%s(): pass = %i, pass_start = %lu, pass_end = "
	...
	for (;; io.size = PAGE_SIZE) {
		...
		ntfs_debug( "%s(): Before: bmp->allocated = 0x%Lx, "
		...
		if (err)
			goto err_ret;
		ntfs_debug( "%s(): Read %lu bytes.\n", __FUNCTION__,
				(unsigned long)io.size);
		ntfs_debug( "%s(): After: bmp->allocated = 0x%Lx, "
				"bmp->size = 0x%Lx, bmp->initialized = "
				"0x%Lx.\n", __FUNCTION__, bmp->allocated,
				bmp->size, bmp->initialized);
		if (!io.size)
		...
		bit = buf_pos & 7UL;
		buf_pos &= ~7UL;
		ntfs_debug( "%s(): Before loop: buf_size = %lu, "
		...
		for (; bit < buf_size && bit + buf_pos < pass_end;
			...
			if (b < (__u8)8 && b >= (bit & 7UL)) {
				...
				ntfs_debug( "%s(): Found free rec "
						"in for loop. bit = %lu\n",
						__FUNCTION__, bit);
				goto found_free_rec;
			}
		}
		ntfs_debug( "%s(): After loop: buf_size = %lu, "
				"buf_pos = %lu, bit = %lu, *byte = 0x%x, b = "
				"%u.\n", __FUNCTION__, buf_size, buf_pos, bit,
				byte ? *byte : -1, b);
		buf_pos += buf_size;
...
pass_done:	/* Finished with the current pass. */
		ntfs_debug( "%s(): At pass_done.\n", __FUNCTION__);
		if (pass == 1) {
			/*
			 * Now do pass 2, scanning the first part of the zone
			 * we omitted in pass 1.
			 */
			ntfs_debug( "%s(): Done pass 1.\n",
					__FUNCTION__);
			ntfs_debug( "%s(): Pass = 2.\n",
					__FUNCTION__);
			pass = 2;
			...
			ntfs_debug( "%s(): pass = %i, pass_start = "
					"%lu, pass_end = %lu.\n", __FUNCTION__,
		...
		if (bmp->initialized << 3 > nr_mft_records &&
			...
			if (bit < 24UL)
				bit = 24UL;
			ntfs_debug( "%s(): Found free record bit "
					"(#1) = 0x%lx.\n", __FUNCTION__, bit);
			goto found_free_rec;
		}
		ntfs_debug( "%s(): Done pass 2.\n", __FUNCTION__);
		ntfs_debug( "%s(): Before: bmp->allocated = 0x%Lx, "
				"bmp->size = 0x%Lx, bmp->initialized = "
				"0x%Lx.\n", __FUNCTION__, bmp->allocated,
				bmp->size, bmp->initialized);
...
rl2_dealloc_err_out:				if (ntfs_deallocate_clusters(
		...
		ntfs_debug( "%s(): Now have sufficient allocated "
				"space in mftbmp.\n", __FUNCTION__);
		ntfs_debug( "%s(): Before: bmp->allocated = 0x%Lx, "
				"bmp->size = 0x%Lx, bmp->initialized = "
				"0x%Lx.\n", __FUNCTION__, bmp->allocated,
		...
		ntfs_debug( "%s(): Wrote extended mftbmp bytes "
				"%lu.\n", __FUNCTION__, (unsigned long)io.size);
		ntfs_debug( "%s(): After write: bmp->allocated = "
				"0x%Lx, bmp->size = 0x%Lx, bmp->initialized = "
				"0x%Lx.\n", __FUNCTION__, bmp->allocated,
				bmp->size, bmp->initialized);
		bit = buf_pos << 3;
		ntfs_debug( "%s(): Found free record bit (#2) = "
				"0x%lx.\n", __FUNCTION__, bit);
		goto found_free_rec;
	}
found_free_rec:
	...
	ntfs_debug( "%s(): At found_free_rec.\n", __FUNCTION__);
	io.param = buf;
	io.size = 1;
	io.do_read = 1;
	ntfs_debug( "%s(): Before update: bmp->allocated = 0x%Lx, "
	...
	*buf |= 1 << (bit & 7);
	...
	ntfs_debug( "%s(): Wrote %lu bytes.\n", __FUNCTION__,
			(unsigned long)io.size);
	ntfs_debug( "%s(): After update: bmp->allocated = 0x%Lx, "
			"bmp->size = 0x%Lx, bmp->initialized = 0x%Lx.\n",
			__FUNCTION__, bmp->allocated,
			bmp->size, bmp->initialized);
	...
	if (ll <= data->initialized) {
		...
		ntfs_debug( "%s(): Allocated mft record "
				"already initialized!\n", __FUNCTION__);
		goto done_ret;
	}
	ntfs_debug( "%s(): Allocated mft record needs "
			"to be initialized.\n", __FUNCTION__);
	...
	while (ll > data->allocated) {
		...
		ntfs_debug( "%s(): Extending mft data allocation, "
		...
		ntfs_debug( "%s(): rl2[r2len - 1].lcn = %i, .len = "
				"%i.\n", __FUNCTION__, rl2[r2len - 1].lcn,
				rl2[r2len - 1].len);
		ntfs_debug( "%s(): lcn2 = %i, r2len = %i.\n",
...
retry_mft_data_allocation:
		...
		ntfs_debug( "%s(): Allocated %i clusters starting "
				"at LCN %i.\n", __FUNCTION__, nr_lcn2, lcn2);
		ntfs_debug( "%s(): Allocated runlist:\n",
				__FUNCTION__);
		dump_runlist(rl2, r2len);
		...
		err = splice_runlists(&data->d.r.runlist, (int*)&data->d.r.len,
				rl2, r2len);
		if (err) {
			ntfs_debug( "%s(): splice_runlists failed "
					"with error code %i.\n", __FUNCTION__,
					-err);
		...
		ntfs_debug( "%s(): After extending mft data "
	...
	while (ll > data->initialized) {
		ntfs_debug( "%s(): Initializing mft record "
				"0x%Lx.\n", __FUNCTION__, 
		...
		ntfs_debug( "%s(): Wrote %i bytes to mft data.\n",
...
done_ret:
	...
	ntfs_debug( "%s(): At done_ret. *result = bit = 0x%lx.\n",
			__FUNCTION__, bit);
	*result = bit;
	vol->mft_data_pos = bit + 1;
err_ret:
	unlock_kernel();
	kfree((unsigned long)buf);
	ntfs_debug( "%s(): Syncing inode $MFT.\n", __FUNCTION__);
	if (ntfs_update_inode(vol->mft_ino))
		ntfs_error("%s(): Failed to sync inode $MFT. "
				"Continuing anyway.\n",__FUNCTION__);
	if (!err) {
		ntfs_debug( "%s(): Done. Allocated mft record "
				"number *result = 0x%lx.\n", __FUNCTION__,
				*result);
		return 0;
	}
...
undo_mftbmp_alloc_err_ret:
	ntfs_debug( "%s(): At undo_mftbmp_alloc_err_ret.\n",
			__FUNCTION__);
...
shrink_mftbmp_err_ret:
	ntfs_debug( "%s(): At shrink_mftbmp_err_ret.\n",
			__FUNCTION__);
	ntfs_debug( "%s(): have_allocated_mftbmp = %i.\n",
