				...
				int used)
{
	...
	if (block == -1) {	/* Index root. */
		...
		if (error) {
			printk("NTFS: ntfs_index_writeback() caught "
		...
		int (*cb)(ntfs_u8 *, void *), void *param)
{
	...
	ntfs_debug( "%s(): Unsorted 1: Entering for inode 0x%lx, "
	...
	if (!*p_high) {
		...
		entry = buf + 0x20;
		ntfs_debug( "%s(): Unsorted 2: In index root.\n",
				__FUNCTION__);
		ibs_bits = ffs(ibs) - 1;
		...
		ntfs_debug( "%s(): Unsorted 3: In index "
				"allocation.\n", __FUNCTION__);
		start = 0;
	}
	/* Process the entries. */
	...
	for (; entry < (buf + ibs) && ntfs_entry_is_used(entry);
		...
		if (start < finish) {
			...
			ntfs_debug( "%s(): Unsorted 4: Skipping "
		...
		//ntfs_debug("%s() dir size=%d\n",__FUNCTION__, NTFS_GETU16(entry + 8));
		thread_wait(current_thread(), 1000);
		ntfs_debug( "%s(): Unsorted 5: Processing entry "
				"p_high 0x%x, p_low 0x%x.\n", __FUNCTION__,
				*p_high, *p_low);
		if ((err = cb(entry, param))) {
			/* filldir signalled us to stop. */
			ntfs_debug( "%s(): Unsorted 6: cb returned "
	...
	ntfs_debug( "%s(): Unsorted 7: After processing entries, "
			"p_high 0x%x, p_low 0x%x.\n", __FUNCTION__, *p_high,
			*p_low);
	...
	*p_low = 0;
	...
	if (!attr) {
		...
		*p_high = 0x7fff;
		ntfs_debug( "%s(): Unsorted 8: No index allocation. "
	...
	if (!attr) {
		ntfs_free(buf);
		ntfs_debug( "%s(): Unsorted 9: Find attr failed. "
				"Returning -EIO.\n", __FUNCTION__);
		return -EIO;
	}
	if (attr->resident) {
		ntfs_free(buf);
		ntfs_debug( "%s(): Unsorted 9.5: IA is resident. Not"
...
fake_eod:
			...
			*p_low = 0;
			ntfs_free(buf);
			ntfs_debug( "%s(): Unsorted 10.5: No more "
	...
	ntfs_debug( "%s(): Unsorted 11: Done. Returning 0, p_high "
