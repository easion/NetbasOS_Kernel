...
#define new_lock( id, name ) ( (((*(id))=create_semaphore( (name), 1, 0 )) < 0) ? -EINVAL : 0 )
#define free_lock(lock) destroy_semaphore( *(lock) )

#define create_sem( count, name ) create_semaphore( (name), (count), 0 )
#define delete_sem		  destroy_semaphore

#define acquire_sem( id )	lock_semaphore( (id) )
#define acquire_sem_etc( id, count, a, b ) lock_semaphore( (id) ) //lock_semaphore_ex( (id), (count), 0 , INFINITE )
#define release_sem_etc( id, count, a )	unlock_semaphore( (id) ) //unlock_semaphore_ex( (id), (count) )
#define release_sem( id ) unlock_semaphore( (id) )

#define	B_MOUNT_READ_ONLY	0x0001

#define strerror(a)		"strerror_not_implemented"

#define time(a)	(*(a) = (startup_ticks() / HZ))
...
{
    uint32	magic;
    int	id;				// ID passed in to fs_mount
...
#define LOCK_VOL(vol) \
	if (vol == NULL) { printk("null vol\n"); return -EINVAL; } else lock_semaphore((vol)->vlock)

#define UNLOCK_VOL(vol) \
	unlock_semaphore((vol)->vlock)
