...
int dosfs_readdir( void *_vol, void *_dir, void *_cookie, int nPos, vfs_dirent_t *entry, size_t bufsize )
{
    int 		result = 0;
    nspace* 	vol = (nspace*)_vol;
...
//			*num = 1;
	    entry->d.l_ino = vol->root_vnode.vnid;
//			entry->d_dev = vol->id;
	    result = 1;
	    goto bi;
	}
    ...
    result = get_next_dirent(vol, dir, &diri, &(entry->d.l_ino), entry->l_long_name, bufsize/* - sizeof(vfs_dirent_t) - 1*/ );
...
//		*num = 1;
	result = 1;
//		entry->d_dev = vol->id;
	entry->l_name_len = strlen(entry->l_long_name);
//		entry->d_reclen = strlen(entry->l_long_name) + 1;
