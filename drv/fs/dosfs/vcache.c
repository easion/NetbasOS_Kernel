...
#define DPRINTF(a,b) if (debug_vcache > (a)) printk b
...
status_t init_vcache(nspace *vol)
{
	...
	sprintf(name, "fat cache %x", vol->id);
...
static status_t _remove_from_vcache_(nspace *vol, ino_t vnid, char check_integrity)
{
