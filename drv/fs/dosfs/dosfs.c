...
static status_t mount_fat_disk(mount_t *mp, int nsid,
		const int flags, nspace** newVol )
{
   ...
   // if ((err = (vol->fd = open(path, O_RDONLY))) < 0) {
    ...
    if (dev_ioctl(vol->fd, IOCTL_GET_DEVICE_GEOMETRY, &geo,sizeof(geo),1) < 0) {
#if 0
	struct stat st;
    ...
    if ((err = dev_read(vol->fd, 0, (void*)buf, 512)) != 512) {
    ...
    if ((err = dev_read(vol->fd, vol->bytes_per_sector*(vol->reserved_sectors + vol->active_fat * vol->sectors_per_fat),
	...
	for (i=0;i<vol->fat_count;i++) {
		...
		buf2[0] = ~buf[0];
		if ((err = dev_read(vol->fd, vol->bytes_per_sector*(vol->reserved_sectors + vol->sectors_per_fat*i), (void *)buf2, 0x200)) != 0x200) {
...
static int dosfs_probe( const mount_t *mp )

{
    ...
    if ((result = mount_fat_disk(mp, -1, 0, &vol)) == B_NO_ERROR) {
	char name[32];
	int  i;
...
int dosfs_mount(mount_t *mp, void *ppVolData )
{
    int	result;
    nspace	*vol;
	int nFsID=1;
...
//    TOUCH(parms); TOUCH(len);
	...
	//return -EINVAL;
      ...
      // Try and mount volume as a FAT volume
    if ((result = mount_fat_disk(mp, nFsID, B_MOUNT_READ_ONLY, &vol)) == B_NO_ERROR) {
	char name[32];


	if (check_nspace_magic(vol, "dosfs_mount")) return -EINVAL;
		
	mp->m_root_ino = vol->root_vnode.vnid;
	mp->m_private_data = (void*)vol;
	tmp_root_inode = iget(mp->m_dev->devno, mp->m_root_ino);
	mp->m_count ++;
...
{
	nspace* vol = (nspace*)mp->m_private_data;
	int i;
...
static  fs_dev_ops_t _dosfs=
{
	fs_name: "dosfs",
	fs_copyright:"Be Sample Code License",
	fs_author:"Be Incorporated",
	fs_readlink:	dosfs_readlink,

	fs_probe:dosfs_probe,
	fs_mount:dosfs_mount,
	fs_unmount:fatfs_unmount,
	//fs_inorw:dosfs_inorw,
	//fs_bmap: dosfs_bmap,
	
	fs_mkdir:dosfs_mkdir,
	//fs_opendir: dosfs_opendir,
	//fs_readdir: dosfs_readdir,

	fs_read:dosfs_read,
	fs_write:dosfs_write,
};
