...
# define	USE_INET6
#endif
#if defined(__netbas__) && (!defined(_KERNEL) || defined(CONFIG_IPV6))
//# define	USE_INET6
#endif
...
/*                            L I N U X                                    */
/* ----------------------------------------------------------------------- */
#if defined(__netbas__) && !defined(OS_RECOGNISED)
...
#  define	IFNAME(x)		((struct netif*)x)->name
#  define	CACHE_HASH(x)	((IFNAME(fin->fin_ifp)[0] + \
			  ((struct netif *)fin->fin_ifp)->ifindex) & 7)
typedef	struct	pbuf	mb_t;
...
static int in_interrupt(){return 1;}
#  define	mbuf	pbuf

#  define	mtod(m, t)	((t)(m)->payload)
#  define	m_data		payload
#  define	m_len		len
#  define	m_next		next
#  define	M_DUPLICATE(m)	skb_clone((m), in_interrupt() ? 0 : \
								1)
...
typedef	struct	{
	...
	int	eMm_held;
	int	eMm_heldat;
#if defined(__hpux) || defined(__netbas__)
	char	eMm_fill[8];
#endif
} eMmutex_t;
...
#if defined(__netbas__) && defined(_KERNEL)
...
# if defined(__NetBSD__) || defined(__OpenBSD__) || defined(__FreeBSD__) || \
     defined(__osf__) || defined(__netbas__)
#  include <netinet/ip6.h>
#  include <netinet/icmp6.h>
