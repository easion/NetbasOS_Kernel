...
static struct device * sis900_mac_probe (struct mac_chip_info mac, pci_state_t pci_dev,
...
struct sis900_private {
	...
	packet_t *tx_skbuff[NUM_TX_DESC];
	packet_t *rx_skbuff[NUM_RX_DESC];
...
static int sis900_start_xmit(packet_t *skb, struct device *net_dev);
...
static int sis900_get_mac_addr(pci_state_t * pci_dev, struct device *net_dev)
{
	long ioaddr = pci_dev->base[0] & ~3;
	uint16 signature;
	int i;
...
static int sis630e_get_mac_addr(pci_state_t * pci_dev, struct device *net_dev)
{
	pci_state_t *isa_bridge = NULL;
	uint8 reg;
	int i;

	if ((isa_bridge = pci_find_device(0x1039, 0x0008)) == NULL) {
		printk("%s: Can not find ISA bridge\n", net_dev->name);
		return 0;
	}
	g_psBus->read_pci_config8(isa_bridge, 0x48, &reg);
	g_psBus->write_pci_config8(isa_bridge, 0x48, reg | 0x40);
...
static int sis635_get_mac_addr(pci_state_t * pci_dev, struct device *net_dev)
{
...
static int sis962_get_mac_addr(pci_state_t * pci_dev, struct device *net_dev)
{
					 ...
					 int device_handle, int found_cnt)
{
	...
	long ioaddr = pci_dev.base[0] & PCI_ADDRESS_IO_MASK;
	int	irq = pci_dev.irq;
	...
	g_psBus->read_pci_config8(sis_priv->pci_dev, PCI_CLASS_REVISION, &revision);
	...
	printk( "%s: %s at %#lx, IRQ %d, ", net_dev->name, sis_priv->mac->name,
	       ioaddr, irq);
	for (i = 0; i < 5; i++)
...
static int sis900_mii_probe (struct device * net_dev)
{
	...
	for (phy_addr = 0; phy_addr < 32; phy_addr++) {	
		...
		if ((mii_phy = kmalloc(sizeof(struct mii_phy), MEMF_KERNEL | MEMF_CLEAR)) == NULL) {
			printk( "Cannot allocate mem for struct mii_phy\n");
			return 0;
		}
		...
		for (i=0; mii_chip_table[i].phy_id1; i++)
			if ( ( mii_phy->phy_id0 == mii_chip_table[i].phy_id0 ) &&
				...
				printk( "%s: %s transceiver found at address %d.\n",
					net_dev->name, mii_chip_table[i].name, phy_addr);
				break;
			}

		if( !mii_chip_table[i].phy_id1 )
			printk( "%s: Unknown PHY transceiver found at address %d.\n",
				net_dev->name, phy_addr);
	}
	
	if (sis_priv->mii == NULL) {
		printk( "%s: No MII transceivers found!\n",
		       net_dev->name);
		return 0;
	}
	...
	/* Reset PHY if default PHY is internal sis900 */
        ...
        if( status & MII_STAT_LINK ){
        	while (poll_bit) {
        		...
        		if (jiffies >= timeout) {
        			printk( "%s: reset phy and link down\n", net_dev->name);
        			return -ETIME;
        		}
        	}
        }
...
static uint16 sis900_default_phy(struct device * net_dev)
{
	...
	if( sis_priv->mii != default_phy ){
		...
		printk( "%s: Using transceiver found at address %d as default\n", net_dev->name,sis_priv->cur_phy);
	}
...
sis900_init_tx_ring(struct device *net_dev)
{
	...
	if (sis900_debug > 2)
		printk( "%s: TX descriptor register loaded with: %8.8x\n",
...
sis900_init_rx_ring(struct device *net_dev)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		...
		if ((skb = new_pkt2(RX_BUF_SIZE,0,0)) == NULL) {
		...
		sis_priv->rx_ring[i].bufptr = virt_to_bus(skb->data); /* tail */
	}
	...
	if (sis900_debug > 2)
		printk( "%s: RX descriptor register loaded with: %8.8x\n",
...
static void sis630_set_eq(struct device *net_dev, uint8 revision)
{
	...
	int i, maxcount = 10;
	pci_state_t *dev = NULL;

	if ( !(revision == SIS630E_900_REV || revision == SIS630EA1_900_REV ||
	       revision == SIS630A_900_REV || revision ==  SIS630ET_900_REV) )
		return;

	if ((dev = pci_find_device(SIS630_VENDOR_ID, SIS630_DEVICE_ID)))
		g_psBus->read_pci_config8(dev, PCI_CLASS_REVISION, &host_bridge_rev);
...
static void sis900_read_mode(struct device *net_dev, int *speed, int *duplex)
{
	...
	*duplex = FDX_CAPABLE_HALF_SELECTED;
	...
	printk( "%s: Media Link On %s %s-duplex \n",
	       net_dev->name,
	       *speed == HW_SPEED_100_MBPS ?
	       "100mbps" : "10mbps",
	       *duplex == FDX_CAPABLE_FULL_SELECTED ?
...
static void sis900_tx_timeout(struct device *net_dev)
{
	...
	printk( "%s: Transmit timeout, status %8.8x %8.8x \n",
...
static int
sis900_start_xmit(packet_t *skb, struct device *net_dev)
{
	struct sis900_private *sis_priv = (struct sis900_private *)net_dev->priv;
	long ioaddr = net_dev->base_addr;
	unsigned int  entry;
	...
	if (sis900_debug > 3)
		printk( "%s: Queued Tx packet at %p size %d "
		       "to slot %d.\n",
...
static int sis900_interrupt(int irq, void *dev_instance, SysCallRegs_s *regs)
{
	...
	if (test_and_set_bit(0, (void*)&net_dev->interrupt)) {
		printk( "%s: SMP simultaneous entry of "
	...
	do {
		...
		if (status & HIBERR) {
			printk( "%s: Abnormal interrupt,"
			       "status %#8.8x.\n", net_dev->name, (int)status);
			break;
		}
		if (--boguscnt < 0) {
			printk( "%s: Too much work at interrupt, "
...
static int sis900_rx(struct device *net_dev)
{
	...
	if (sis900_debug > 3)
		printk( "sis900_rx, cur_rx:%4.4d, dirty_rx:%4.4d "
		       "status:0x%8.8x\n",
	...
	while (rx_status & OWN) {
		...
		if (rx_status & (ABORT|OVERRUN|TOOLONG|RUNT|RXISERR|CRCERR|FAERR)) {
			...
			sis_priv->rx_ring[entry].cmdsts = RX_BUF_SIZE;
			sis_priv->rx_ring[entry].bufptr = virt_to_bus(skb->data);
			sis_priv->dirty_rx++;
		}
		sis_priv->cur_rx++;
...
int sis900_probe()
{
    ...
    for ( i = 0 ; g_psBus->get_pci_info( &sInfo, i ) == 0 ; ++i ) {
		...
		ioaddr = sInfo.base[0] & PCI_ADDRESS_IO_MASK;
		irq = sInfo.irq;
...
static status_t sis900_ioctl(int fd, uint32 nCommand, void* pArgs, bool bFromKernel )
{
	void* pNode=NULL;//fix me
...
static int sis900_read( int fd, off_t nPosition, void* pBuffer, size_t nSize )
{
    return( -ENOSYS );
}

static int sis900_write(int fd,  off_t nPosition, const void* pBuffer, size_t nSize )
{
	void* pNode=NULL;
	//fix me
	struct device* dev = pNode;
	packet_t* psBuffer = new_pkt2( nSize,0,0 );
	if ( psBuffer != NULL ) {
		memcpy( psBuffer->data, pBuffer, nSize );
...
static const driver_ops_t sis900_drv_fops =
{
	d_name:		"sis900",
	d_author:	"Jim Huang <cmhuang@sis.com.tw>, Ollie Lho <ollie@sis.com.tw> ",
	d_version:	"2006-10-29",
	d_index:	SIS900_MARJOR,
	open:		sis900_open_dev,
	close:		sis900_close_dev,
	read:		sis900_read,
	write:		sis900_write,
	ioctl:		sis900_ioctl,		
...
status_t device_init()
{
	...
	if( g_psBus == NULL )
	{
		printk("pci bus_hooks not setup\n");
		return( -1 );
	}	
 
	retval=kernel_driver_register(&sis900_drv_fops);
		
	 if(retval < 0)
    {
      printk("Could not register sis900 device\n");
      return 0;
    }
