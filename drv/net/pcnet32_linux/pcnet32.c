...
/* port to Netnas OS by Easion(easion@gmail.com)*/
...
#include <drv/drv.h>
#include <drv/sym.h>
static dev_prvi_t* pcnet32_devfp;

#define pb_pData payload
#define pb_nSize len
...
int pcnet32_probe(  );
static struct net_device* pcnet32_probe1(   long , int , int , int );
static int  pcnet32_open(struct net_device *);
static int  pcnet32_init_ring(struct net_device *);
static int  pcnet32_start_xmit(struct pbuf *, struct net_device *);
static int  pcnet32_rx(struct net_device *);
static int  pcnet32_interrupt( void *, int,SysCallRegs_s *);
...
struct pcnet32_pci_id_info
{
   ...
   int io_size;
   struct net_device *(*probe1) (   long , int , int , int );
};
...
int pci_pcnet32_probe(device_t self)
{
   ...
   do
	...
	ops->read_pci_config16(  sInfo,
			       PCI_SUBSYSTEM_VENDOR_ID, &sdid );
	ops->read_pci_config16( sInfo,
			       PCI_SUBSYSTEM_ID, &svid );

	for ( chip_idx = 0 ; pcnet32_tbl[chip_idx].vendor_id ; chip_idx++ )
	  if( (sInfo->vendorid == pcnet32_tbl[chip_idx].vendor_id) &&
	     (sInfo->deviceid == pcnet32_tbl[chip_idx].device_id) &&
	...
	/*if( claim_device( device_handle, sInfo->nHandle, "PCnet PCI", DEVICE_NET ) != 0 )
		...
		dev = pcnet32_tbl[chip_idx].probe1(  ioaddr, irq, 1, cards_found);
...
static struct net_device*
  pcnet32_probe1( long ioaddr, int irq, int shared, int cards_found)
{
...
static int pcnet32_init_ring(struct net_device *dev)
{
   ...
   for (i = 0; i < RX_RING_SIZE; i++)
	...
	if (lp->rx_skbuff[i] == NULL)
	  {
	     if (!(lp->rx_skbuff[i] = pbuf_alloc(PBUF_RAW, ETHER_FRAME_LEN, PBUF_RW)))
	       {
...
static int pcnet32_start_xmit(struct pbuf *skb, struct net_device *dev)
{
   struct pcnet32_private *lp = (struct pcnet32_private *)dev->priv;
...
static int pcnet32_interrupt( void *dev_id,int irq, SysCallRegs_s * regs)
{
   ...
   while ((csr0 = lp->a.read_csr (ioaddr, 0)) & 0x8600 && --boguscnt >= 0)
	...
	if (csr0 & 0x0800)
	  {
	     printk("%s: Bus master arbitration failure, status %x.\n",
		    dev->name, csr0);	
		  ...
		  if (pkt_len > rx_copybreak)
		    {
		       ...
		       if ((newskb = pbuf_alloc(PBUF_RAW, ETHER_FRAME_LEN, PBUF_RW)))
			 {			    
		  ...
		  else
		    {
		       ...
		       //skb->pb_nSize = 0;
			   skb = pbuf_alloc(PBUF_RAW, ETHER_FRAME_LEN, PBUF_RW);
...
static int opened=0;
status_t pcnet32_drv_close( void* pNode, void* pCookie )
{
	opened = 0;
   return( 0 );
}
...
{
   struct net_device* psDev = EthDevice;
...
status_t pcnet32_ioctl(dev_prvi_t* devfp, uint32 nCommand, void* pArgs,int len, bool bFromKernel )
{
   struct net_device* psDev = EthDevice;
   int nError = 0;
...
int pcnet32_write( dev_prvi_t* devfp, off_t nPosition, const void* pBuffer, size_t nSize )
{
   /*struct net_device* dev = EthDevice;
   struct pbuf* psBuffer = alloc_pkt_buffer( nSize );
   if ( psBuffer != NULL )
     {
...
int pcnet32_transmit(dev_prvi_t* devfp, struct pbuf *p)
{
	...
	if (len > 0)
	{
		err=pcnet32_start_xmit( q, EthDevice );
		if (err){
			return -1;
		}
	}
...
int pcnet32_attach(dev_prvi_t* devfp, struct eth_addr *hwaddr)
{
	...
	//kprintf("get mac address\n");
	memcpy( (hwaddr->addr),	psDev->dev_addr, MAX_ADDR_LEN );
  //*hwaddr = pcnet32->hwaddr;

  //*hwaddr = pcnet32->hwaddr;

  kprintf("pcnet32.c mac address %2x-%2x-%2x-%2x-%2x-%2x\n" ,
	  hwaddr->addr[0],
	  hwaddr->addr[1],
	  hwaddr->addr[2],
	  hwaddr->addr[3],
