...
/*
	...
	//2006-12-17
	Ported to Netbas OS by Easion <easion@gmail.com>
*/
...
struct device* EthDevice=0;
char* get_mac()
{
	return EthDevice->dev_addr;
}

packetqueue_t* get_packetqueue()
{
	assert(EthDevice!=NULL);
	assert(EthDevice->packet_queue!=NULL);
	return EthDevice->packet_queue;
}
...
struct rtl8129_private {
	...
	void *timer;	/* Media selection timer. */
	int		 tx_sema;
...
static int rtl8129_start_xmit(packet_t *skb, struct net_device *dev);
static int rtl8129_rx(struct net_device *dev);
static int rtl8129_interrupt(void *arg, int irq, void *regs);
...
//	unsigned char pci_bus, pci_device_fn;
	...
	ops = (pci_pos_t*)bus->bus_hooks(0);
	if (!ops)
	{
		return -3;
	}

    sInfo = ops->pci_dev_lookup(0x02, 0x00);
    ...
    do {
		...
		for ( chip_idx = 0 ; pci_tbl[chip_idx].vendor_id ; chip_idx++ ) {
			//sInfo = pci_find_device(pci_tbl[chip_idx].vendor_id,
			//	pci_tbl[chip_idx].device_id,NULL);
			if (sInfo->vendorid == pci_tbl[chip_idx].vendor_id 
				&&sInfo->deviceid == pci_tbl[chip_idx].device_id)
			{
				break;
			}
...
#define PCI_ADDRESS_IO_MASK	0xFFFFFFFC	/* mask to get i/o space base address */
		...
		ops->read_pci_config16( sInfo, PCI_COMMAND, &pci_command );
		new_command = pci_command | (pci_tbl[chip_idx].pci_flags & 7);
		if (pci_command != new_command) {
			printk(KERN_INFO "  The PCI BIOS has not enabled the"
				   " device at %d/%d/%d!  Updating PCI command %4.4x->%4.4x.\n",
				   0, 0, 
					   0, pci_command, new_command );
			ops->write_pci_config16( sInfo, PCI_COMMAND,  new_command);
		}

	 for (i=0; i<6; i++)	{
		if(sInfo->base[i])
		  break;
       // nic->iobase = sInfo->base[i];
	  }

		dev = rtl8129_probe1( sInfo->bus, sInfo->dev_fn,
			sInfo->func, 
			sInfo->hdr_type, sInfo->interface,
			sInfo->base[i], sInfo->irq, chip_idx, cards_found );
										
		//set_device_data( sInfo.nHandle, dev ); //绑定
									   ...
									   int irq, int chip_idx, int found_cnt)
{
	...
	printk(KERN_INFO "%s: %s at %x, IRQ %d\n",
	...
	spinlock_init( &np->tx_spinlock,"rtl8139_card" );
...
static int rtl8129_open(struct net_device *dev)
{
	...
	//MOD_INC_USE_COUNT;
	dev->irq_handle = request_irq(dev->irq, rtl8129_interrupt, 
	...
	add_timer(&tp->timer);
	*/
	tp->timer = (void*)kmalloc(sizeof(krnl_timer_t), 0);
	init_timer( tp->timer, (timer_callback *) &rtl8129_timer,  dev);
	install_timer(tp->timer,500);
...
static void rtl8129_timer(void* data)
{
	...
	restart_timer( np->timer, next_tick * 10);
	/*
...
static int
rtl8129_start_xmit(packet_t *skb, struct net_device *dev)
{
	struct rtl8129_private *tp = (struct rtl8129_private *)dev->priv;
	long ioaddr = dev->base_addr;
	int entry;
	...
	syslog(LOG_WARNING, __FUNCTION__"() add to entry %d \n", entry);
	...
	while ((inb(ioaddr + ChipCmd) & RxBufEmpty) == 0) {
		...
		if (rx_status & (RxBadSymbol|RxRunt|RxTooLong|RxCRCErr|RxBadAlign)) {
			...
			if ( dev->packet_queue != NULL ) {
				...
				queue_pkt_add( dev->packet_queue, skb );
				//syslog(LOG_DEBUG, __FUNCTION__" enqueue_packet %d succ\n",dev->packet_queue->count);
				wake_up_eththread();
				//dump_packet(skb);
			} else {
...
rtl8129_close(struct net_device *dev)
{
	...
	mm_free((unsigned int)tp->rx_ring,tp->rx_buf_len + 16 +
...
int rtl8129_ioctl( int fd, uint32 nCommand, void* pArgs, int len,bool bFromKernel )
{
	struct device* psDev = EthDevice;
    int nError = 0;
    ...
    switch( nCommand )
    {
		case SIOC_ETH_START:
		{
			psDev->packet_queue = new_pktque();
			if (!psDev->packet_queue)
			{
				nError = -1;
				break;
			}
		...
		case SIOC_ETH_STOP:
		{
			queue_pkt_free(psDev->packet_queue);
			rtl8129_close( psDev );
		...
		case SIOCSIFHWADDR:
			...
			break;
		case SIOCGIFHWADDR:
			memcpy( ((void*)pArgs),	psDev->dev_addr, 6 );
			break;
		default:
...
{
	struct device* dev ;
	packet_t* psBuffer = alloc_pkt_buffer( nSize );
	dev = EthDevice;
	//syslog(LOG_WARNING, __FUNCTION__"() size is %d \n", nSize);
	if ( psBuffer != NULL ) {
		memcpy( psBuffer->pb_pData, pBuffer, nSize );
		//memcpy( skb_put(psBuffer->pb_pData,nSize), pBuffer, nSize );
...
*/
int device_suspend( int nDeviceID, int nDeviceHandle, void* pData )
{
...
int device_resume( int nDeviceID, int nDeviceHandle, void* pData )
{
	struct net_device* dev = pData;
	long ioaddr = dev->base_addr;
	struct rtl8129_private *tp = (struct rtl8129_private *)dev->priv;
