...
struct dhcp_msg
{
  ...
  u8_t htype;			  // Hardware addr type
  u8_t hlen;			  // Hardware addr length
  u8_t hops;			  // Number of relay agent hops from client
  u32_t xid;			  // Transaction ID
  u16_t secs;			  // Seconds since client started looking
  u16_t flags;			  // Flag bits
  struct in_addr ciaddr;		  // Client IP address (if already in use)
  struct in_addr yiaddr;		  // Client IP address
  struct in_addr siaddr;		  // IP address of next server to talk to
  struct in_addr giaddr;		  // DHCP relay agent IP address
  u8_t chaddr[DHCP_CHADDR_LEN];  // Client hardware address
  char sname[DHCP_SNAME_LEN];		  // Server name
  char file[DHCP_FILE_LEN];		  // Boot filename
  u32_t cookie;                   // DHCP option cookie
  u8_t options[0];   		  // Optional parameters (actual length dependent on MTU).
}__attr_packet;
...
struct dhcp_state
{
  ...
  struct thread_sem * binding_complete;
};
